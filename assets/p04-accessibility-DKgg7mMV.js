...
<section class="p4-hero section-dark section-hero-full">
  <div class="flex-col-center" style="gap:var(--space-md);text-align:center;position:relative;z-index:1;">
    <p class="hero-eyebrow p4-eyebrow" style="opacity:0;">Module 01 / Page 04</p>
    <h1 class="page-hero-title p4-hero-title" style="color:var(--text-on-dark);opacity:0;">色彩与阅读无障碍</h1>
    <p class="page-hero-sub p4-hero-sub" style="opacity:0;">Color Accessibility</p>
    <p class="p4-hero-tagline" style="font-family:var(--font-body);font-size:var(--text-body);color:var(--text-on-dark-2);max-width:540px;line-height:1.8;margin-top:var(--space-sm);opacity:0;">
      好的科研图表不只是"好看"——它需要被每一位读者清晰阅读。<br>
      从对比度到字号，从配色到排版，让你的研究触达所有人。
    </p>
    <nav class="hero-quicknav" id="p4-hero-nav" style="opacity:0;">
    ...
    <div class="p4-scroll-hint" style="opacity:0;">↓ 向下探索</div>
  </div>
</section>
...
</div>`}function rt(){const t=F.timeline({delay:.2});t.fromTo(".p4-eyebrow",{opacity:0,y:20},{opacity:1,y:0,duration:.6,ease:"power3.out"},0),t.fromTo(".page-hero-title",{y:30,opacity:0},{opacity:1,y:0,duration:.8,ease:"power3.out"},.15),t.fromTo(".page-hero-sub",{y:20,opacity:0},{opacity:.5,y:0,duration:.8,ease:"power3.out"},.3),t.fromTo(".p4-hero-tagline",{y:20,opacity:0},{opacity:1,y:0,duration:.8,ease:"power3.out"},.45),t.fromTo("#p4-hero-nav",{y:20,opacity:0},{opacity:1,y:0,duration:.8,ease:"power3.out"},.6),t.fromTo(".p4-scroll-hint",{opacity:0,y:15},{opacity:1,y:0,duration:.6,ease:"power3.out"},.75),m(".p4-contrast-card",{y:40}),m("#p4-typo-section .p4-section-header",{y:40}),R(".p4-typo-controls"),R(".p4-typo-preview"),m("#p4-font-section .p4-section-header",{y:40}),m(".p4-font-compare",{y:30}),m(".p4-scale-table-wrap",{y:30}),m(".p4-lh-compare",{y:30}),m(".p4-journal-table-wrap",{y:30}),m("#p4-check-section .p4-section-header",{y:40}),F.utils.toArray(".p4-check-card").forEach((i,v)=>{m(i,{y:30})}),m("#p4-footer",{y:30}),document.querySelectorAll("#p4-hero-nav .hero-quicknav__item").forEach(i=>{i.addEventListener("click",()=>{const v=document.querySelector(i.dataset.target);v&&v.scrollIntoView({behavior:"smooth",block:"start"})})});try{U()}catch(i){console.error("p04 setupContrastChecker:",i)}try{J()}catch(i){console.error("p04 setupChartTypography:",i)}try{V()}catch(i){console.error("p04 setupNavButtons:",i)}}function U(){const t=document.getElementById("p4-fg-picker"),i=document.getElementById("p4-bg-picker"),v=document.getElementById("p4-fg-hex"),S=document.getElementById("p4-bg-hex"),w=document.getElementById("p4-fg-swatch"),u=document.getElementById("p4-bg-swatch");function y(){const r=N(a.fgColor,a.bgColor),g=r.toFixed(2);document.getElementById("p4-ratio-num").textContent=g;const b=r>=4.5,l=r>=3,f=r>=7;k("p4-badge-aa",b,"AA 正文"),k("p4-badge-aa-large",l,"AA 大字"),k("p4-badge-aaa",f,"AAA");const s=Math.min(100,r/21*100),h=document.getElementById("p4-bar-fill");h&&(h.style.width=s+"%",h.style.background=r>=4.5?"var(--color-success)":r>=3?"#F0B27A":"var(--color-error)");const A=document.getElementById("p4-preview-box");A&&(A.style.background=a.bgColor,A.style.color=a.fgColor),w&&(w.style.background=a.fgColor),u&&(u.style.background=a.bgColor)}function k(r,g,b){const l=document.getElementById(r);l&&(l.className=`p4-badge ${g?"p4-badge--pass":"p4-badge--fail"}`,l.textContent=`${g?"✓":"✗"} ${b}`)}function z(r,g,b,l){!r||!g||(r.addEventListener("input",f=>{const s=f.target.value;l?a.fgColor=s:a.bgColor=s,g.value=s.toUpperCase(),y()}),g.addEventListener("input",f=>{let s=f.target.value.trim();s.startsWith("#")||(s="#"+s),/^#[0-9A-Fa-f]{6}$/.test(s)&&(l?a.fgColor=s:a.bgColor=s,r.value=s,y())}))}z(t,v,!1,!0),z(i,S,!1,!1);const c=document.getElementById("p4-font-dec"),p=document.getElementById("p4-font-inc"),d=document.getElementById("p4-font-size-val"),e=document.getElementById("p4-preview-normal");function o(){d&&(d.textContent=a.previewFontSize+"px"),e&&(e.style.fontSize=a.previewFontSize+"px")}c&&c.addEventListener("click",()=>{a.previewFontSize=Math.max(8,a.previewFontSize-1),o()}),p&&p.addEventListener("click",()=>{a.previewFontSize=Math.min(32,a.previewFontSize+1),o()}),y()}function J(){const t=document.getElementById("p4-chart-canvas");if(!t)return;const i=[{group:"Control",a:23,b:18,c:15},{group:"Treatment A",a:35,b:28,c:22},{group:"Treatment B",a:41,b:32,c:27},{group:"Treatment C",a:29,b:24,c:19}],v=["a","b","c"],S=["Gene α","Gene β","Gene γ"],w=["#7EC8E3","#5BA3C9","#3A7CA5"];function u(){t.innerHTML="";const c=t.getBoundingClientRect(),p=c.width||600,d=c.height||450,e=a.chart,o={top:20+e.titleSize+e.subtitleSize+8,right:100,bottom:20+e.axisLabelSize+e.tickSize+8,left:20+e.axisLabelSize+e.tickSize+8},r=W(t).append("svg").attr("viewBox",`0 0 ${p} ${d}`).attr("preserveAspectRatio","xMidYMid meet");r.append("rect").attr("width",p).attr("height",d).attr("fill","#ffffff");const g=p-o.left-o.right,b=d-o.top-o.bottom,l=r.append("g").attr("transform",`translate(${o.left},${o.top})`),f=j().domain(i.map(n=>n.group)).range([0,g]).padding(.25),s=j().domain(v).range([0,f.bandwidth()]).padding(.05),h=q().domain([0,50]).range([b,0]);l.selectAll(".grid-line").data(h.ticks(5)).enter().append("line").attr("x1",0).attr("x2",g).attr("y1",n=>h(n)).attr("y2",n=>h(n)).attr("stroke","#e8e8e8").attr("stroke-dasharray","3,3"),l.selectAll(".bar-group").data(i).enter().append("g").attr("transform",n=>`translate(${f(n.group)},0)`).selectAll("rect").data(n=>v.map(x=>({key:x,value:n[x]}))).enter().append("rect").attr("x",n=>s(n.key)).attr("y",n=>h(n.value)).attr("width",s.bandwidth()).attr("height",n=>b-h(n.value)).attr("fill",(n,x)=>w[x]).attr("rx",2),l.append("g").attr("transform",`translate(0,${b})`).call(G(f).tickSize(0)).selectAll("text").attr("font-size",e.tickSize+"px").attr("fill","#555"),l.select(".domain").attr("stroke","#ccc"),l.append("g").call(O(h).ticks(5).tickSize(-4)).selectAll("text").attr("font-size",e.tickSize+"px").attr("fill","#555"),r.append("text").attr("x",o.left+g/2).attr("y",d-6).attr("text-anchor","middle").attr("font-size",e.axisLabelSize+"px").attr("font-weight",600).attr("fill","#333").text("实验组"),r.append("text").attr("x",-(o.top+b/2)).attr("y",14).attr("transform","rotate(-90)").attr("text-anchor","middle").attr("font-size",e.axisLabelSize+"px").attr("font-weight",600).attr("fill","#333").text("表达量 (FPKM)"),r.append("text").attr("x",o.left).attr("y",e.titleSize+4).attr("font-size",e.titleSize+"px").attr("font-weight",e.titleWeight).attr("fill","#1d1d1f").text("三组基因在不同处理下的表达量"),r.append("text").attr("x",o.left).attr("y",e.titleSize+e.subtitleSize+10).attr("font-size",e.subtitleSize+"px").attr("fill","#888").text("数据来源：RNA-seq, n = 3 replicates");const L=o.top+10,M=p-o.right+16,B=r.append("g").attr("transform",`translate(${M},${L})`);v.forEach((n,x)=>{const $=x*(e.legendSize+10);B.append("rect").attr("x",0).attr("y",$).attr("width",e.legendSize).attr("height",e.legendSize).attr("fill",w[x]).attr("rx",2),B.append("text").attr("x",e.legendSize+6).attr("y",$+e.legendSize*.8).attr("font-size",e.legendSize+"px").attr("fill","#555").text(S[x])});const I=i[2],T=f(I.group)+s("a")+s.bandwidth()/2,_=h(I.a)-8;r.append("text").attr("x",o.left+T).attr("y",o.top+_).attr("text-anchor","middle").attr("font-size",e.annotationSize+"px").attr("font-style","italic").attr("fill","#888").text("p < 0.01 *")}const y=[{id:"p4-c-title",valId:"p4-v-title",key:"titleSize",unit:"px"},{id:"p4-c-subtitle",valId:"p4-v-subtitle",key:"subtitleSize",unit:"px"},{id:"p4-c-axis",valId:"p4-v-axis",key:"axisLabelSize",unit:"px"},{id:"p4-c-tick",valId:"p4-v-tick",key:"tickSize",unit:"px"},{id:"p4-c-legend",valId:"p4-v-legend",key:"legendSize",unit:"px"},{id:"p4-c-annotation",valId:"p4-v-annotation",key:"annotationSize",unit:"px"}];y.forEach(c=>{const p=document.getElementById(c.id),d=document.getElementById(c.valId);p&&p.addEventListener("input",e=>{const o=parseInt(e.target.value);a.chart[c.key]=o,d&&(d.textContent=o+c.unit),u()})});const k=document.getElementById("p4-typo-reset");k&&k.addEventListener("click",()=>{a.chart={...E},y.forEach(c=>{const p=document.getElementById(c.id),d=document.getElementById(c.valId);p&&(p.value=a.chart[c.key]),d&&(d.textContent=a.chart[c.key]+c.unit)}),u()}),u();const z=new ResizeObserver(()=>u());z.observe(t),a._resizeObserver=z}function V(){const t=document.getElementById("p4-next-btn"),i=document.getElementById("p4-prev-btn"),v=document.getElementById("p4-m1-btn");t&&t.addEventListener("click",()=>C("m1-p5")),i&&i.addEventListener("click",()=>C("m1-p3")),v&&v.addEventListener("click",()=>C("m1"))}function ot(){H(),a._resizeObserver&&(a._resizeObserver.disconnect(),a._resizeObserver=null),a={fgColor:"#1d1d1f",bgColor:"#ffffff",previewFontSize:16,chart:{...E},lineHeightA:1.2,lineHeightB:1.8}}export{ot as destroy,rt as init,at as render};
