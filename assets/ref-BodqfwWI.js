...
`;function k(){return v.map(e=>`
  ...
  `).join("")}function B(){return b.map(e=>`
  ...
  `}function f(e){const t=document.getElementById("ref-toast");t&&(t.textContent=e,t.classList.add("ref-toast-show"),i&&clearTimeout(i),i=setTimeout(()=>{t.classList.remove("ref-toast-show")},1200))}function m(e){if(navigator.clipboard)navigator.clipboard.writeText(e).then(()=>{f(`已复制 ${e}`)});else{const t=document.createElement("textarea");t.value=e,t.style.position="fixed",t.style.opacity="0",document.body.appendChild(t),t.select(),document.execCommand("copy"),document.body.removeChild(t),f(`已复制 ${e}`)}}function h(e){const t=document.getElementById("ref-content");if(!t)return;const a={charts:k,palettes:B,journals:F,export:A,shortcuts:T}[e];a&&(d.killTweensOf(t),t.innerHTML=a(),d.fromTo(t,{opacity:0},{opacity:1,duration:.3,ease:"power2.out"}),C(e,t))}function C(e,t){e==="palettes"&&t.querySelectorAll(".ref-swatch").forEach(a=>{const o=()=>m(a.dataset.color);a.addEventListener("click",o),n.push({el:a,type:"click",fn:o})}),e==="export"&&t.querySelectorAll(".ref-copy-btn").forEach(a=>{const o=()=>m(a.dataset.copy);a.addEventListener("click",o),n.push({el:a,type:"click",fn:o})})}function P(){const e=d.timeline({delay:.2});e.fromTo(".ref-hero .hero-eyebrow",{opacity:0,y:20},{opacity:1,y:0,duration:.6,ease:"power3.out"},0),e.fromTo(".ref-hero .page-hero-title",{y:30,opacity:0},{opacity:1,y:0,duration:.8,ease:"power3.out"},.15),e.fromTo(".ref-hero .page-hero-sub",{y:20,opacity:0},{opacity:.5,y:0,duration:.8,ease:"power3.out"},.3),e.fromTo(".ref-hero-tagline",{y:20,opacity:0},{opacity:1,y:0,duration:.8,ease:"power3.out"},.45),e.fromTo(".ref-scroll-hint",{opacity:0,y:15},{opacity:1,y:0,duration:.6,ease:"power3.out"},.6);const t=document.getElementById("ref-tabs-mount");t&&(s=g(t,{tabs:[{id:"charts",label:"图表选择"},{id:"palettes",label:"配色方案"},{id:"journals",label:"期刊规格"},{id:"export",label:"导出格式"},{id:"shortcuts",label:"快捷键"}],activeId:"charts",variant:"pill",onChange:o=>{h(o)}}),h("charts")),l("#ref-tabs-section",{y:40,duration:.8}),l(".page-footer-quote",{y:40,duration:.9}),l(".page-footer-cta .page-footer-nav",{y:25,duration:.6});const r=document.getElementById("ref-prev-btn"),a=document.getElementById("ref-next-btn");r&&p(r,"click",()=>c("m4-p4")),a&&p(a,"click",()=>c("home"))}function I(){u(),n.forEach(({el:e,type:t,fn:r,opts:a})=>{e.removeEventListener(t,r,a)}),n=[],s&&(s.destroy(),s=null),i&&(clearTimeout(i),i=null)}export{I as destroy,P as init,$ as render};
