...
# transparent=True`,r:`ggsave(
    ...
    dpi = 300
        ...
        <pre style="background:#1d1d1f;color:#f5f5f7;padding:12px 14px;border-radius:8px;font-size:12px;white-space:pre-wrap;word-wrap:break-word;overflow-wrap:break-word;margin:0;font-family:var(--font-code);line-height:1.5">${w.data.api}</pre>`)}),p.filter(e=>e.data.id==="Figure").dispatch("click")}function D(t){let a=t;return()=>(a=a*1664525+1013904223&4294967295,(a>>>0)/4294967295)}function zt(t,a,l,b,n){t.selectAll("*").remove();const m={t:25,r:20,b:35,l:45},d=b-m.l-m.r,s=n-m.t-m.b,p=t.append("g").attr("transform",`translate(${m.l},${m.t})`);t.insert("rect","g").attr("width",b).attr("height",n).attr("fill","#1d1d1f").attr("rx",8);for(let o=1;o<=4;o++)p.append("line").attr("x1",0).attr("y1",o*s/4).attr("x2",d).attr("y2",o*s/4).attr("stroke","#2d2d2f").attr("stroke-width",1);p.append("line").attr("x1",0).attr("y1",s).attr("x2",d).attr("y2",s).attr("stroke","#424245").attr("stroke-width",1),p.append("line").attr("x1",0).attr("y1",0).attr("x2",0).attr("y2",s).attr("stroke","#424245").attr("stroke-width",1);const e=D(42),w=["A","B","C","D"],x={deep:["#4C72B0","#DD8452","#55A868","#C44E52"],Set2:["#66C2A5","#FC8D62","#8DA0CB","#E78AC3"],Set1:["#E41A1C","#377EB8","#4DAF4A","#984EA3"],pastel:["#AEC6CF","#FFD1DC","#B5EAD7","#FFDAC1"],viridis:["#440154","#31688E","#35B779","#FDE725"],muted:["#4878D0","#EE854A","#6ACC64","#D65F5F"],rocket:["#03051A","#4A0C5C","#B0298B","#F87060"],husl:["#F77189","#BB9832","#50B131","#36ADA4"]},g=l.palette&&x[l.palette]||["#7EC8E3","#95D5B2","#B8B8E8","#F0B27A"];function y(o,c,h,f="middle"){p.append("text").attr("x",o).attr("y",c).attr("text-anchor",f).attr("font-size",10).attr("fill","#a0a0a8").attr("font-family","sans-serif").text(h)}if(a==="scatter"){const o=Array.from({length:50},()=>({x:.05+e()*.9,y:.05+e()*.9,gi:Math.floor(e()*4)})),c=Math.max(2,Math.sqrt((l.size||60)/Math.PI)*1.1);p.selectAll("circle").data(o).join("circle").attr("cx",h=>h.x*d).attr("cy",h=>(1-h.y)*s).attr("r",c).attr("fill",h=>l.hue?g[h.gi]:g[0]).attr("opacity",l.alpha||.7)}else if(a==="line"){const c=Array.from({length:3},(f,r)=>({gi:r,pts:Array.from({length:10},(v,E)=>({x:E/9,y:Math.max(.02,Math.min(.92,.12+r*.22+e()*.08+Math.sin(E/3)*.06))}))})),h=T().x(f=>f.x*d).y(f=>(1-f.y)*s).curve(L);c.forEach(f=>{if(l.ci){const r=X().x(v=>v.x*d).y0(v=>(1-Math.max(.02,v.y-.06))*s).y1(v=>(1-Math.min(.92,v.y+.06))*s).curve(L);p.append("path").attr("d",r(f.pts)).attr("fill",g[f.gi]).attr("opacity",.2)}p.append("path").attr("d",h(f.pts)).attr("fill","none").attr("stroke",g[f.gi]).attr("stroke-width",2),l.markers&&p.selectAll(`.mk${f.gi}`).data(f.pts).join("circle").attr("class",`.mk${f.gi}`).attr("cx",r=>r.x*d).attr("cy",r=>(1-r.y)*s).attr("r",3.5).attr("fill",g[f.gi])}),[0,3,6,9].forEach(f=>y(f/9*d,s+14,`Q${f+1}`))}else if(a==="bar"){const o=w.map(()=>.35+e()*.45),c=o.map(h=>h*.1+.03);if(l.orient==="v"){const h=W().domain(w).range([0,d]).padding(.28);p.selectAll("rect").data(o).join("rect").attr("x",(f,r)=>h(w[r])).attr("y",f=>(1-f)*s).attr("width",h.bandwidth()).attr("height",f=>f*s).attr("fill",(f,r)=>g[r]).attr("rx",3),l.ci&&o.forEach((f,r)=>{const v=h(w[r])+h.bandwidth()/2,E=(1-(f+c[r]))*s,i=(1-(f-c[r]))*s;p.append("line").attr("x1",v).attr("y1",E).attr("x2",v).attr("y2",i).attr("stroke","#fff").attr("stroke-width",1.5),p.append("line").attr("x1",v-5).attr("y1",E).attr("x2",v+5).attr("y2",E).attr("stroke","#fff").attr("stroke-width",1.5),p.append("line").attr("x1",v-5).attr("y1",i).attr("x2",v+5).attr("y2",i).attr("stroke","#fff").attr("stroke-width",1.5)}),w.forEach((f,r)=>y(h(f)+h.bandwidth()/2,s+14,f))}else{const h=W().domain(w).range([0,s]).padding(.28);p.selectAll("rect").data(o).join("rect").attr("x",0).attr("y",(f,r)=>h(w[r])).attr("width",f=>f*d).attr("height",h.bandwidth()).attr("fill",(f,r)=>g[r]).attr("rx",3),l.ci&&o.forEach((f,r)=>{const v=h(w[r])+h.bandwidth()/2,E=(f+c[r])*d;p.append("line").attr("x1",E).attr("y1",v-6).attr("x2",E).attr("y2",v+6).attr("stroke","#fff").attr("stroke-width",1.5)}),w.forEach((f,r)=>y(-6,h(f)+h.bandwidth()/2+4,f,"end"))}}else if(a==="box"){const o=(l.width||.5)*d/5.5,c=!!l.notch;w.forEach((h,f)=>{const r=(f+.5)*(d/4),v=.35+e()*.28,E=Math.max(.04,v-.1-e()*.06),i=Math.min(.94,v+.1+e()*.06),u=Math.max(.01,E-.13),k=Math.min(.97,i+.13),$=g[f];if(p.append("line").attr("x1",r).attr("y1",(1-k)*s).attr("x2",r).attr("y2",(1-i)*s).attr("stroke",$).attr("stroke-width",1.5).attr("stroke-dasharray","3,2"),p.append("line").attr("x1",r).attr("y1",(1-E)*s).attr("x2",r).attr("y2",(1-u)*s).attr("stroke",$).attr("stroke-width",1.5).attr("stroke-dasharray","3,2"),p.append("line").attr("x1",r-o/3).attr("y1",(1-k)*s).attr("x2",r+o/3).attr("y2",(1-k)*s).attr("stroke",$).attr("stroke-width",1.5),p.append("line").attr("x1",r-o/3).attr("y1",(1-u)*s).attr("x2",r+o/3).attr("y2",(1-u)*s).attr("stroke",$).attr("stroke-width",1.5),c){const A=o*.45,z=(1-v)*s,_=(1-E)*s,S=(1-i)*s,B=`M${r-o/2},${S} L${r-o/2},${z-7} L${r-A/2},${z} L${r-o/2},${z+7} L${r-o/2},${_} L${r+o/2},${_} L${r+o/2},${z+7} L${r+A/2},${z} L${r+o/2},${z-7} L${r+o/2},${S}Z`;p.append("path").attr("d",B).attr("fill",$).attr("opacity",.7)}else p.append("rect").attr("x",r-o/2).attr("y",(1-i)*s).attr("width",o).attr("height",(i-E)*s).attr("fill",$).attr("opacity",.7).attr("rx",2);p.append("line").attr("x1",r-o/2).attr("y1",(1-v)*s).attr("x2",r+o/2).attr("y2",(1-v)*s).attr("stroke","#fff").attr("stroke-width",2),y(r,s+14,h)})}else if(a==="violin"){const o=Math.max(.4,Math.min(2.5,l.bw||1)),c=l.inner||"box";w.forEach((h,f)=>{const r=(f+.5)*(d/4),v=g[f],E=28,i=[];for(let S=0;S<E;S++){const B=S/(E-1),P=.05+B*.9,Q=Math.sin(Math.PI*B),K=Math.sin(Math.PI*B*1.6+f*.6)*.18,U=Math.max(1,(Q+K)*(.035+e()*.018)*o*d);i.push({y:P,w:U})}const u=i.map((S,B)=>`${B===0?"M":"L"}${r-S.w},${(1-S.y)*s}`).join(" ")+i.slice().reverse().map(S=>`L${r+S.w},${(1-S.y)*s}`).join(" ")+"Z";p.append("path").attr("d",u).attr("fill",v).attr("opacity",.65);const k=.38+e()*.22,$=Math.max(.06,k-.1),A=Math.min(.92,k+.1),z=Math.floor(E/2),_=i[z].w*.45;if(c==="box")p.append("rect").attr("x",r-_/2).attr("y",(1-A)*s).attr("width",_).attr("height",(A-$)*s).attr("fill","#1d1d1f").attr("stroke",v).attr("stroke-width",1),p.append("circle").attr("cx",r).attr("cy",(1-k)*s).attr("r",3).attr("fill","#fff");else if(c==="quartile")[$,k,A].forEach(S=>{p.append("line").attr("x1",r-_).attr("y1",(1-S)*s).attr("x2",r+_).attr("y2",(1-S)*s).attr("stroke",S===k?"#fff":"#aaa").attr("stroke-width",S===k?2:1)});else if(c==="point")for(let S=0;S<12;S++){const B=.08+e()*.84,P=(e()-.5)*_*1.8;p.append("circle").attr("cx",r+P).attr("cy",(1-B)*s).attr("r",1.5).attr("fill","#fff").attr("opacity",.75)}else if(c==="stick")for(let S=0;S<12;S++){const B=.08+e()*.84;p.append("line").attr("x1",r-_*.9).attr("y1",(1-B)*s).attr("x2",r+_*.9).attr("y2",(1-B)*s).attr("stroke","#fff").attr("stroke-width",.8).attr("opacity",.7)}y(r,s+14,h)})}else if(a==="hist"){const o=Math.max(4,Math.min(l.bins||20,50)),c=.45,h=.18,f=Array.from({length:o},(v,E)=>{const i=(E+.5)/o;return Math.exp(-Math.pow((i-c)/h,2)/2)*(.7+e()*.18)}),r=d/o;if(p.selectAll("rect").data(f).join("rect").attr("x",(v,E)=>E*r+1).attr("y",v=>(1-v)*s).attr("width",r-2).attr("height",v=>v*s).attr("fill",l.color||"#7EC8E3").attr("opacity",.85),l.kde){const v=Array.from({length:60},(E,i)=>{const u=i/59;return{x:u,y:Math.exp(-Math.pow((u-c)/h,2)/2)*.92}});p.append("path").attr("d",T().x(E=>E.x*d).y(E=>(1-E.y)*s).curve(q)(v)).attr("fill","none").attr("stroke","#F0D264").attr("stroke-width",2.5)}}else if(a==="kde"){const c=.12*Math.max(.3,l.bw||1),h=l.hue?3:1,f=[.28,.48,.68];for(let r=0;r<h;r++){const v=g[r],E=Array.from({length:60},(u,k)=>{const $=k/59;return{x:$,y:Math.exp(-Math.pow(($-f[r])/c,2)/2)*.88}}),i=T().x(u=>u.x*d).y(u=>(1-u.y)*s).curve(q);if(l.fill){const u=X().x(k=>k.x*d).y0(s).y1(k=>(1-k.y)*s).curve(q);p.append("path").attr("d",u(E)).attr("fill",v).attr("opacity",.2)}p.append("path").attr("d",i(E)).attr("fill","none").attr("stroke",v).attr("stroke-width",2.5)}}else if(a==="heatmap"){const c=d/4,h=s/4,f=[[1,.72,-.31,.54],[.72,1,.18,-.08],[-.31,.18,1,-.63],[.54,-.08,-.63,1]],v=ot({coolwarm:R,viridis:pt,RdBu:R,Blues:mt,Oranges:bt}[l.cmap]||R).domain([1,-1]),E=["X1","X2","X3","X4"];for(let i=0;i<4;i++)for(let u=0;u<4;u++)if(p.append("rect").attr("x",u*c+1).attr("y",i*h+1).attr("width",c-2).attr("height",h-2).attr("fill",v(f[i][u])).attr("rx",2),l.annot){const k=l.fmt===".2f"?2:1;p.append("text").attr("x",u*c+c/2).attr("y",i*h+h/2+4).attr("text-anchor","middle").attr("font-size",9).attr("fill","#fff").attr("font-weight","600").attr("font-family","JetBrains Mono, monospace").text(f[i][u].toFixed(k))}E.forEach((i,u)=>y(u*c+c/2,s+14,i))}else if(a==="pair"){const c=d/2,h=s/2,f=l.hue?3:1;for(let r=0;r<2;r++)for(let v=0;v<2;v++)if(p.append("rect").attr("x",v*c+2).attr("y",r*h+2).attr("width",c-4).attr("height",h-4).attr("fill","#2d2d2f").attr("rx",4),r===v)if(l.diag==="kde")for(let E=0;E<f;E++){const i=.25+E*.25,u=Array.from({length:20},(k,$)=>{const A=$/19;return{x:A,y:Math.exp(-Math.pow((A-i)/.16,2)/2)*.85}});p.append("path").attr("d",T().x(k=>v*c+4+k.x*(c-8)).y(k=>r*h+h-4-k.y*(h-8)).curve(q)(u)).attr("fill","none").attr("stroke",g[E]).attr("stroke-width",1.5)}else for(let i=0;i<f;i++)for(let u=0;u<6;u++){const k=(.15+e()*.55)*(h-8)*.8,$=(c-8)/6/f;p.append("rect").attr("x",v*c+4+u*(c-8)/6+i*$).attr("y",r*h+h-4-k).attr("width",$-1).attr("height",k).attr("fill",g[i]).attr("opacity",.85)}else for(let E=0;E<f;E++)for(let i=0;i<8;i++)p.append("circle").attr("cx",v*c+5+e()*(c-10)).attr("cy",r*h+5+e()*(h-10)).attr("r",2.5).attr("fill",l.hue?g[E]:g[0]).attr("opacity",.75);["V1","V2"].forEach((r,v)=>y(v*c+c/2,s+14,r))}else if(a==="reg"){const o=l.order||1,c=l.color||"#7EC8E3",h=(l.ci||95)/100*.07,f=Array.from({length:35},()=>({x:.05+e()*.9,y:0}));f.forEach(i=>{o===1?i.y=.1+i.x*.7+(e()-.5)*.16:o===2?i.y=.1+.1*i.x+.9*i.x*i.x+(e()-.5)*.13:i.y=.7-2.2*i.x+3.5*i.x*i.x-1.5*i.x*i.x*i.x+(e()-.5)*.1,i.y=Math.max(.02,Math.min(.95,i.y))}),l.scatter&&p.selectAll("circle").data(f).join("circle").attr("cx",i=>i.x*d).attr("cy",i=>(1-i.y)*s).attr("r",3.5).attr("fill",c).attr("opacity",.45);const r=Array.from({length:50},(i,u)=>{const k=.05+u/49*.9;let $;return o===1?$=.1+k*.7:o===2?$=.1+.1*k+.9*k*k:$=.7-2.2*k+3.5*k*k-1.5*k*k*k,{x:k,y:Math.max(.02,Math.min(.95,$))}}),v=r.map(i=>({x:i.x*d,y:(1-Math.min(.95,i.y+h))*s})),E=[...r].reverse().map(i=>({x:i.x*d,y:(1-Math.max(.02,i.y-h))*s}));p.append("path").attr("d",v.map((i,u)=>`${u===0?"M":"L"}${i.x},${i.y}`).join(" ")+" "+E.map(i=>`L${i.x},${i.y}`).join(" ")+"Z").attr("fill",c).attr("opacity",.2),p.append("path").attr("d",T().x(i=>i.x*d).y(i=>(1-i.y)*s).curve(L)(r)).attr("fill","none").attr("stroke",c).attr("stroke-width",2.5)}}function Mt(t){if(!t)return null;const a=t.querySelector(".sb-list"),l=t.querySelector(".sb-params"),b=t.querySelector(".sb-preview"),n=t.querySelector(".sb-code");if(!a||!l||!b||!n)return null;let m=I[0],d=Object.fromEntries(m.params.map(x=>[x.id,x.default])),s=null;I.forEach((x,g)=>{const y=document.createElement("div");y.className="sb-list-item",y.style.cssText="display:flex;gap:10px;align-items:center;padding:10px 12px;cursor:pointer;border-radius:8px;transition:background 0.2s;min-height:44px;min-width:0;",y.innerHTML=`<span style="font-size:16px;flex-shrink:0">${x.icon}</span><div style="min-width:0;overflow:hidden"><div style="font-weight:600;font-size:13px;color:var(--text-on-dark);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${x.name}</div><div style="font-size:11px;color:var(--text-on-dark-2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${x.desc}</div></div>`,g===0&&(y.style.background="rgba(126,200,227,0.12)"),y.addEventListener("click",()=>{a.querySelectorAll(".sb-list-item").forEach(o=>o.style.background=""),y.style.background="rgba(126,200,227,0.12)",m=x,d=Object.fromEntries(x.params.map(o=>[o.id,o.default])),p(),e(),w()}),a.appendChild(y)});function p(){l.innerHTML=`<div style="font-weight:600;font-size:14px;color:var(--text-on-dark);margin-bottom:14px;font-family:var(--font-code)">${m.name}</div>`,m.params.forEach(x=>{const g=document.createElement("div");g.style.marginBottom="12px",x.type==="select"?g.innerHTML=`<label style="display:block;margin-bottom:4px;font-size:12px;color:var(--text-on-dark-2)">${x.label}</label>
