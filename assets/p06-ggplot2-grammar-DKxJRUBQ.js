...
<section class="p6-layers-section" id="p6-layers">
  ...
  <div class="p6-mobile-steps-list" id="p6-mobile-steps-list">
    ...
    <div class="p6-mobile-step-card">
      <div class="p6-step-num">Step ${s+1} / ${A.length}</div>
      <h3 class="p6-step-title">${o.title}</h3>
      <p class="p6-step-desc">${o.desc}</p>
      <div class="p6-code-panel" id="p6-m-code-${s}"></div>
      <div class="p6-m-chart-box" id="p6-m-chart-${s}"></div>
    </div>`).join("")}
  </div>
  ...
  <div class="p6-layers-body" id="p6-layers-body">
    ...
    <div class="p6-sticky-steps" id="p6-sticky-steps">
      ${A.map((o,s)=>`
      <div class="p6-step-panel" data-step="${s}">
...
</div>`}function T(o,s){o.innerHTML="";const x=400,f=300,e={t:24,r:20,b:50,l:54},a=x-e.l-e.r,r=f-e.t-e.b,n=z(o).append("svg").attr("viewBox",`0 0 ${x} ${f}`).attr("preserveAspectRatio","xMidYMid meet");if(n.append("rect").attr("width",x).attr("height",f).attr("fill","#111218"),s===0){n.append("text").attr("x",x/2).attr("y",f/2).attr("text-anchor","middle").attr("dominant-baseline","middle").attr("fill","#444").attr("font-family","var(--font-heading)").attr("font-size","13px").text("ggplot(iris)  ← 空白画布");return}const p=[4,8.2],i=[.3,7.4],l=E().domain(p).range([0,a]),t=E().domain(i).range([r,0]),d=n.append("g").attr("transform",`translate(${e.l},${e.t})`),g=s>=4?"#191c2a":"#1a1d2e";if(d.selectAll(".gy").data(t.ticks(5)).join("line").attr("x1",0).attr("x2",a).attr("y1",m=>t(m)).attr("y2",m=>t(m)).attr("stroke",g).attr("stroke-width",1),s<4&&d.selectAll(".gym").data(t.ticks(10)).join("line").attr("x1",0).attr("x2",a).attr("y1",m=>t(m)).attr("y2",m=>t(m)).attr("stroke","#161820").attr("stroke-width",.5),d.append("g").attr("transform",`translate(0,${r})`).call($(l).ticks(5)).call(m=>{m.select(".domain").attr("stroke","#555"),m.selectAll("text").attr("fill",s>=1?"#888":"#444").attr("font-size","9px"),m.selectAll("line").attr("stroke","#555")}),d.append("g").call(H(t).ticks(5)).call(m=>{m.select(".domain").attr("stroke","#555"),m.selectAll("text").attr("fill",s>=1?"#888":"#444").attr("font-size","9px"),m.selectAll("line").attr("stroke","#555")}),s>=1&&(d.append("text").attr("x",a/2).attr("y",r+38).attr("text-anchor","middle").attr("fill","#999").attr("font-size","10.5px").attr("font-family","var(--font-code)").text("Sepal.Length"),d.append("text").attr("transform","rotate(-90)").attr("x",-r/2).attr("y",-42).attr("text-anchor","middle").attr("fill","#999").attr("font-size","10.5px").attr("font-family","var(--font-code)").text("Petal.Length")),s>=2&&d.selectAll("circle").data(B).join("circle").attr("cx",m=>l(m.x)).attr("cy",m=>t(m.y)).attr("r",3.5).attr("fill",m=>s>=3?k[m.sp]:"#7EC8E3").attr("opacity",s>=3?.82:.55),s>=2){const m=["setosa","versicolor","virginica"],y=s>=4,w=n.append("g");y?m.forEach((u,_)=>{const c=e.l+_*100+10,v=e.t+r+44;w.append("circle").attr("cx",c).attr("cy",v).attr("r",4).attr("fill",k[u]),w.append("text").attr("x",c+8).attr("y",v+1).attr("dominant-baseline","middle").attr("fill","#aaa").attr("font-size","9px").attr("font-family","var(--font-heading)").text(u)}):m.forEach((u,_)=>{const c=e.t+_*18+6,v=e.l+a+8;w.append("circle").attr("cx",v).attr("cy",c).attr("r",4).attr("fill",k[u]),w.append("text").attr("x",v+8).attr("y",c+1).attr("dominant-baseline","middle").attr("fill","#aaa").attr("font-size","9px").attr("font-family","var(--font-heading)").text(u)})}}function C(o,s){o&&(o.innerHTML=A[s].lines.map(x=>`<div class="p6-code-line${x.new?" hl":""}">${Y(x.text)}</div>`).join(""))}function Y(o){return o.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function K(o,s){o.innerHTML="";const x=["setosa","versicolor","virginica"];if(s==="none"){const a={t:20,r:16,b:46,l:50},r=420-a.l-a.r,n=290-a.t-a.b,p=z(o).append("svg").attr("viewBox","0 0 420 290").attr("preserveAspectRatio","xMidYMid meet");p.append("rect").attr("width",420).attr("height",290).attr("fill","#111218");const i=p.append("g").attr("transform",`translate(${a.l},${a.t})`),l=E().domain([4,8.3]).range([0,r]),t=E().domain([.3,7.4]).range([n,0]);q(i,l,t,r,n,"#1e2030"),at(i,l,t,r,n),et(p,a,r,n,"Sepal.Length","Petal.Length"),i.selectAll("circle").data(B).join("circle").attr("cx",d=>l(d.x)).attr("cy",d=>t(d.y)).attr("r",3.5).attr("fill",d=>k[d.sp]).attr("opacity",.8),x.forEach((d,g)=>{p.append("circle").attr("cx",a.l+r-76).attr("cy",a.t+g*18+8).attr("r",4).attr("fill",k[d]),p.append("text").attr("x",a.l+r-69).attr("y",a.t+g*18+8).attr("dominant-baseline","middle").attr("fill","#aaa").attr("font-size","9px").text(d)})}else if(s==="facet_wrap"){const a={t:18,r:10,b:32,l:36},r=10,n=540+2*r+12,p=183,i=z(o).append("svg").attr("viewBox",`0 0 ${n} ${p}`).attr("preserveAspectRatio","xMidYMid meet");i.append("rect").attr("width",n).attr("height",p).attr("fill","#111218"),x.forEach((l,t)=>{const d=6+t*(180+r),g=22,m=B.filter(v=>v.sp===l),y=180-a.l-a.r,w=155-a.t-a.b,u=E().domain([4,8.3]).range([0,y]),_=E().domain([.3,7.4]).range([w,0]);i.append("rect").attr("x",d).attr("y",g).attr("width",180).attr("height",155).attr("fill","#161820").attr("rx",5),i.append("text").attr("x",d+180/2).attr("y",g-5).attr("text-anchor","middle").attr("fill",k[l]).attr("font-size","9px").attr("font-family","var(--font-heading)").text(l);const c=i.append("g").attr("transform",`translate(${d+a.l},${g+a.t})`);q(c,u,_,y,w,"#1e2030"),c.append("g").attr("transform",`translate(0,${w})`).call($(u).ticks(3)).call(v=>{v.select(".domain").attr("stroke","#444"),v.selectAll("text").attr("fill","#666").attr("font-size","7px"),v.selectAll("line").attr("stroke","#444")}),c.append("g").call(H(_).ticks(3)).call(v=>{v.select(".domain").attr("stroke","#444"),v.selectAll("text").attr("fill","#666").attr("font-size","7px"),v.selectAll("line").attr("stroke","#444")}),c.selectAll("circle").data(m).join("circle").attr("cx",v=>u(v.x)).attr("cy",v=>_(v.y)).attr("r",3).attr("fill",k[l]).attr("opacity",.85)})}else{const f=["small (≤5.8)","large (>5.8)"],r={t:14,r:8,b:26,l:32},n=8,p=8,i=310+n+70,l=336+2*p+36,t=z(o).append("svg").attr("viewBox",`0 0 ${i} ${l}`).attr("preserveAspectRatio","xMidYMid meet");t.append("rect").attr("width",i).attr("height",l).attr("fill","#111218"),f.forEach((d,g)=>{t.append("text").attr("x",44+g*(155+n)+155/2).attr("y",13).attr("text-anchor","middle").attr("fill","#aaa").attr("font-size","8px").text(d)}),x.forEach((d,g)=>{t.append("text").attr("transform",`rotate(90, ${i-5}, ${24+g*(112+p)+112/2})`).attr("x",i-5).attr("y",24+g*(112+p)+112/2).attr("text-anchor","middle").attr("dominant-baseline","middle").attr("fill",k[d]).attr("font-size","8px").text(d),f.forEach((m,y)=>{const w=y===1,u=B.filter(b=>b.sp===d&&(w?b.x>5.8:b.x<=5.8)),_=44+y*(155+n),c=20+g*(112+p),v=155-r.l-r.r,L=112-r.t-r.b,M=E().domain([4,8.3]).range([0,v]),I=E().domain([.3,7.4]).range([L,0]);t.append("rect").attr("x",_).attr("y",c).attr("width",155).attr("height",112).attr("fill","#161820").attr("rx",4);const W=t.append("g").attr("transform",`translate(${_+r.l},${c+r.t})`);q(W,M,I,v,L,"#1e2030"),g===2&&W.append("g").attr("transform",`translate(0,${L})`).call($(M).ticks(3)).call(b=>{b.select(".domain").attr("stroke","#444"),b.selectAll("text").attr("fill","#666").attr("font-size","7px"),b.selectAll("line").attr("stroke","#444")}),y===0&&W.append("g").call(H(I).ticks(3)).call(b=>{b.select(".domain").attr("stroke","#444"),b.selectAll("text").attr("fill","#666").attr("font-size","7px"),b.selectAll("line").attr("stroke","#444")}),W.selectAll("circle").data(u).join("circle").attr("cx",b=>M(b.x)).attr("cy",b=>I(b.y)).attr("r",2.5).attr("fill",k[d]).attr("opacity",.85)})})}}function Q(o,s){if(o.innerHTML="",s==="cartesian"){const e={t:24,r:16,b:52,l:52},a=380-e.l-e.r,r=280-e.t-e.b,n=z(o).append("svg").attr("viewBox","0 0 380 280").attr("preserveAspectRatio","xMidYMid meet");n.append("rect").attr("width",380).attr("height",280).attr("fill","#fafafa");const p=n.append("g").attr("transform",`translate(${e.l},${e.t})`),i=F().domain(S.map(t=>t.cat)).range([0,a]).padding(.38),l=E().domain([0,38]).range([r,0]);l.ticks(5).forEach(t=>{p.append("line").attr("x1",0).attr("x2",a).attr("y1",l(t)).attr("y2",l(t)).attr("stroke","#ebebeb").attr("stroke-width",1)}),p.append("g").attr("transform",`translate(0,${r})`).call($(i).tickSize(0)).call(t=>{t.select(".domain").attr("stroke","#ccc"),t.selectAll("text").attr("fill","#555").attr("font-size","10px")}),p.append("g").call(H(l).ticks(5)).call(t=>{t.select(".domain").attr("stroke","#ccc"),t.selectAll("text").attr("fill","#555").attr("font-size","10px"),t.selectAll("line").attr("stroke","#ccc")}),p.selectAll("rect.b").data(S).join("rect").attr("class","b").attr("x",t=>i(t.cat)).attr("width",i.bandwidth()).attr("y",t=>l(t.val)).attr("height",t=>r-l(t.val)).attr("fill",t=>k[t.cat]).attr("rx",4).attr("opacity",.88),p.selectAll("text.v").data(S).join("text").attr("class","v").attr("x",t=>i(t.cat)+i.bandwidth()/2).attr("y",t=>l(t.val)-6).attr("text-anchor","middle").attr("fill","#333").attr("font-size","10.5px").attr("font-weight","600").text(t=>t.val.toFixed(1)),n.append("text").attr("x",e.l+a/2).attr("y",276).attr("text-anchor","middle").attr("fill","#aaa").attr("font-size","10px").text("coord_cartesian()（默认）")}else if(s==="flip"){const e={t:14,r:52,b:38,l:88},a=380-e.l-e.r,r=250-e.t-e.b,n=z(o).append("svg").attr("viewBox","0 0 380 250").attr("preserveAspectRatio","xMidYMid meet");n.append("rect").attr("width",380).attr("height",250).attr("fill","#fafafa");const p=n.append("g").attr("transform",`translate(${e.l},${e.t})`),i=F().domain(S.map(t=>t.cat)).range([0,r]).padding(.38),l=E().domain([0,38]).range([0,a]);l.ticks(5).forEach(t=>{p.append("line").attr("y1",0).attr("y2",r).attr("x1",l(t)).attr("x2",l(t)).attr("stroke","#ebebeb").attr("stroke-width",1)}),p.append("g").attr("transform",`translate(0,${r})`).call($(l).ticks(5)).call(t=>{t.select(".domain").attr("stroke","#ccc"),t.selectAll("text").attr("fill","#555").attr("font-size","10px"),t.selectAll("line").attr("stroke","#ccc")}),p.append("g").call(H(i).tickSize(0)).call(t=>{t.select(".domain").attr("stroke","#ccc"),t.selectAll("text").attr("fill","#555").attr("font-size","10px")}),p.selectAll("rect.b").data(S).join("rect").attr("class","b").attr("y",t=>i(t.cat)).attr("height",i.bandwidth()).attr("x",0).attr("width",t=>l(t.val)).attr("fill",t=>k[t.cat]).attr("rx",4).attr("opacity",.88),p.selectAll("text.v").data(S).join("text").attr("class","v").attr("y",t=>i(t.cat)+i.bandwidth()/2).attr("x",t=>l(t.val)+5).attr("dominant-baseline","middle").attr("fill","#333").attr("font-size","10px").attr("font-weight","600").text(t=>t.val.toFixed(1)),n.append("text").attr("x",e.l+a/2).attr("y",246).attr("text-anchor","middle").attr("fill","#aaa").attr("font-size","10px").text("coord_flip()")}else{const e=z(o).append("svg").attr("viewBox","0 0 300 300").attr("preserveAspectRatio","xMidYMid meet");e.append("rect").attr("width",300).attr("height",300).attr("fill","#fafafa");const a=300/2,r=300/2+8,n=96;[.33,.67,1].forEach(l=>{e.append("circle").attr("cx",a).attr("cy",r).attr("r",l*n).attr("fill","none").attr("stroke","#ebebeb").attr("stroke-width",1)});const p=J(S,l=>l.val);let i=-Math.PI/2;S.forEach(l=>{const t=l.val/p*2*Math.PI,d=i+t,g=l.val/38*n,m=U()({innerRadius:0,outerRadius:g,startAngle:i,endAngle:d});e.append("path").attr("transform",`translate(${a},${r})`).attr("d",m).attr("fill",k[l.cat]).attr("opacity",.85).attr("stroke","#fafafa").attr("stroke-width",2);const y=i+t/2;e.append("text").attr("x",a+(g+18)*Math.sin(y)).attr("y",r-(g+18)*Math.cos(y)).attr("text-anchor","middle").attr("dominant-baseline","middle").attr("fill","#444").attr("font-size","9px").text(l.cat),i=d}),e.append("text").attr("x",300/2).attr("y",296).attr("text-anchor","middle").attr("fill","#aaa").attr("font-size","10px").text('coord_polar(theta = "y")')}}function Z(o,{breaks:s,axisMin:x,axisMax:f,customLabels:e}){o.innerHTML="";const a=480,r=320,n={t:24,r:20,b:56,l:54},p=a-n.l-n.r,i=r-n.t-n.b,l=z(o).append("svg").attr("viewBox",`0 0 ${a} ${r}`).attr("preserveAspectRatio","xMidYMid meet");l.append("rect").attr("width",a).attr("height",r).attr("fill","#111218").attr("rx",8);const t=l.append("g").attr("transform",`translate(${n.l},${n.t})`),d=[Math.min(x,f-.5),Math.max(f,x+.5)],g=E().domain(d).range([0,p]),m=E().domain([0,7.5]).range([i,0]);m.ticks(5).forEach(c=>{t.append("line").attr("x1",0).attr("x2",p).attr("y1",m(c)).attr("y2",m(c)).attr("stroke","#1e2030").attr("stroke-width",1)}),g.ticks(s).forEach(c=>{t.append("line").attr("x1",g(c)).attr("x2",g(c)).attr("y1",0).attr("y2",i).attr("stroke","#1e2030").attr("stroke-width",1)});const y="rgba(126,200,227,0.35)";[0,p].forEach(c=>{t.append("line").attr("x1",c).attr("x2",c).attr("y1",0).attr("y2",i).attr("stroke",y).attr("stroke-width",1.5).attr("stroke-dasharray","4,3")});const w=g.ticks(s),u=$(g).tickValues(w);if(e){const c=["短","↓","中","↓","中长","↓","長","↓","超长"];u.tickFormat((v,L)=>c[L%c.length]||v.toFixed(1))}t.append("g").attr("transform",`translate(0,${i})`).call(u).call(c=>{c.select(".domain").attr("stroke","#555"),c.selectAll("text").attr("fill","#888").attr("font-size","10px"),c.selectAll("line").attr("stroke","#555")}),t.append("g").call(H(m).ticks(5)).call(c=>{c.select(".domain").attr("stroke","#555"),c.selectAll("text").attr("fill","#888").attr("font-size","10px"),c.selectAll("line").attr("stroke","#555")}),t.append("text").attr("x",p/2).attr("y",i+44).attr("text-anchor","middle").attr("fill","#777").attr("font-size","11px").attr("font-family","var(--font-code)").text("Sepal.Length"),t.append("text").attr("transform","rotate(-90)").attr("x",-i/2).attr("y",-42).attr("text-anchor","middle").attr("fill","#777").attr("font-size","11px").attr("font-family","var(--font-code)").text("Petal.Length");const _=B.filter(c=>c.x>=d[0]&&c.x<=d[1]);t.selectAll("circle").data(_).join("circle").attr("cx",c=>g(c.x)).attr("cy",c=>m(c.y)).attr("r",3.5).attr("fill",c=>k[c.sp]).attr("opacity",.82)}function tt(o,s,x,f){return`scale_x_continuous(
  breaks = seq(${s}, ${x},
    length.out = ${o}),${f?`
  labels = c("短","中","長"),`:""}
  limits = c(${s}, ${x})
)`}function q(o,s,x,f,e,a){o.selectAll(".gy").data(x.ticks(5)).join("line").attr("x1",0).attr("x2",f).attr("y1",r=>x(r)).attr("y2",r=>x(r)).attr("stroke",a).attr("stroke-width",1)}function at(o,s,x,f,e){o.append("g").attr("transform",`translate(0,${e})`).call($(s).ticks(5)).call(a=>{a.select(".domain").attr("stroke","#555"),a.selectAll("text").attr("fill","#888").attr("font-size","9px"),a.selectAll("line").attr("stroke","#555")}),o.append("g").call(H(x).ticks(5)).call(a=>{a.select(".domain").attr("stroke","#555"),a.selectAll("text").attr("fill","#888").attr("font-size","9px"),a.selectAll("line").attr("stroke","#555")})}function et(o,s,x,f,e,a){o.append("text").attr("x",s.l+x/2).attr("y",s.t+f+40).attr("text-anchor","middle").attr("fill","#888").attr("font-size","10px").attr("font-family","var(--font-code)").text(e),o.append("text").attr("transform","rotate(-90)").attr("x",-132).attr("y",12).attr("text-anchor","middle").attr("fill","#888").attr("font-size","10px").attr("font-family","var(--font-code)").text(a)}function R(o){return Y(o).replace(/\b(ggplot|aes|geom_point|geom_col|scale_color_manual|scale_fill_manual|facet_wrap|facet_grid|theme_minimal|theme|coord_flip|coord_polar|scale_x_continuous|vars|seq|element_blank)\b/g,'<span class="fn">$1</span>').replace(/\b(c|TRUE|FALSE|NULL)\b/g,'<span class="kw">$1</span>').replace(/&quot;([^&]*)&quot;/g,'<span class="str">&quot;$1&quot;</span>').replace(/\b(\d+\.?\d*)\b/g,'<span class="num">$1</span>')}function mt(){document.querySelectorAll("#p6-hero-nav .hero-quicknav__item").forEach(e=>{e.addEventListener("click",()=>{const a=document.querySelector(e.dataset.target);a&&a.scrollIntoView({behavior:"smooth",block:"start"})})});const o=D.timeline({delay:.1});if(o.fromTo(".p6-eyebrow",{opacity:0,y:20},{opacity:1,y:0,duration:.7,ease:"power3.out"},0),o.fromTo(".p6-hero-title",{opacity:0,y:40},{opacity:1,y:0,duration:.9,ease:"power3.out"},.15),o.fromTo(".p6-hero-sub",{opacity:0,y:30},{opacity:.5,y:0,duration:.8,ease:"power3.out"},.3),o.fromTo(".p6-hero-tagline",{opacity:0,y:20},{opacity:1,y:0,duration:.8,ease:"power3.out"},.45),o.fromTo("#p6-hero-nav",{opacity:0,y:20},{opacity:1,y:0,duration:.8,ease:"power3.out"},.6),window.innerWidth>768){let r=function(){if(!e||!a)return;const n=e.getBoundingClientRect(),p=Math.max(0,-n.top),i=Math.max(0,e.offsetHeight-a.offsetHeight);a.style.transform=`translateY(${Math.min(p,i)}px)`;const l=window.innerHeight,t=Math.min(A.length-1,Math.max(0,Math.floor(p/l)));t!==h.layerStep&&(h.layerStep=t,C(document.getElementById("p6-code-display"),t),T(document.getElementById("p6-layer-chart"),t),document.querySelectorAll(".p6-step-panel").forEach((d,g)=>{d.classList.toggle("p6-step-active",g===t)}))};C(document.getElementById("p6-code-display"),0),T(document.getElementById("p6-layer-chart"),0),document.querySelectorAll(".p6-step-panel")[0]?.classList.add("p6-step-active");const e=document.getElementById("p6-layers-body"),a=document.getElementById("p6-sticky-left");window.addEventListener("scroll",r,{passive:!0}),h.scrollHandlers.push({el:window,fn:r}),r()}window.innerWidth<=768&&A.forEach((e,a)=>{const r=document.getElementById(`p6-m-code-${a}`),n=document.getElementById(`p6-m-chart-${a}`);r&&C(r,a),n&&T(n,a)});function s(e){h.facetTab=e,K(document.getElementById("p6-facet-chart"),e);const a=document.getElementById("p6-facet-code");a&&(a.innerHTML=R(N[e])),document.querySelectorAll("#p6-facet-tabs .p6-tab-btn").forEach(r=>{r.classList.toggle("active",r.dataset.facet===e)})}document.querySelectorAll("#p6-facet-tabs .p6-tab-btn").forEach(e=>{e.addEventListener("click",()=>s(e.dataset.facet))}),s("none");function x(e){h.coordTab=e,Q(document.getElementById("p6-coord-chart"),e);const a=document.getElementById("p6-coord-code");a&&(a.innerHTML=R(X[e])),document.querySelectorAll("#p6-coord-tabs .p6-coord-tab-btn").forEach(r=>{r.classList.toggle("active",r.dataset.coord===e)})}document.querySelectorAll("#p6-coord-tabs .p6-coord-tab-btn").forEach(e=>{e.addEventListener("click",()=>x(e.dataset.coord))}),x("cartesian");function f(){const e=parseInt(document.getElementById("p6-breaks-range")?.value||"5"),a=parseFloat(document.getElementById("p6-min-range")?.value||"4"),r=parseFloat(document.getElementById("p6-max-range")?.value||"8");h.axisBreaks=e,h.axisMin=a,h.axisMax=r;const n=document.getElementById("p6-breaks-val"),p=document.getElementById("p6-min-val"),i=document.getElementById("p6-max-val");n&&(n.textContent=e),p&&(p.textContent=a.toFixed(1)),i&&(i.textContent=r.toFixed(1)),Z(document.getElementById("p6-axis-chart"),{breaks:e,axisMin:a,axisMax:r,customLabels:h.customLabels});const l=document.getElementById("p6-axis-code");l&&(l.innerHTML=R(tt(e,a,r,h.customLabels)))}["p6-breaks-range","p6-min-range","p6-max-range"].forEach(e=>{document.getElementById(e)?.addEventListener("input",f)}),document.getElementById("p6-labels-auto")?.addEventListener("click",()=>{h.customLabels=!1,document.getElementById("p6-labels-auto")?.classList.add("active"),document.getElementById("p6-labels-custom")?.classList.remove("active"),f()}),document.getElementById("p6-labels-custom")?.addEventListener("click",()=>{h.customLabels=!0,document.getElementById("p6-labels-custom")?.classList.add("active"),document.getElementById("p6-labels-auto")?.classList.remove("active"),f()}),f(),P(document.querySelectorAll(".p6-sec-hdr"),{stagger:.12,y:50}),P(document.querySelectorAll(".page-footer-quote, .page-footer-desc"),{stagger:.15}),G(document.getElementById("p6-axis-layout")),document.getElementById("p6-next-btn")?.addEventListener("click",()=>j("m1-p7")),document.getElementById("p6-back-btn")?.addEventListener("click",()=>j("m1-p5"))}function xt(){O(),h.layerObservers.forEach(o=>o.disconnect()),h.layerObservers=[],h.scrollHandlers.forEach(({el:o,fn:s})=>o.removeEventListener("scroll",s)),h.scrollHandlers=[],h.resizeHandlers.forEach(o=>window.removeEventListener("resize",o)),h.resizeHandlers=[],h.layerStep=0,h.facetTab="none",h.coordTab="cartesian",h.customLabels=!1}export{xt as destroy,mt as init,dt as render};
