...
</div>`}function at(){w.fromTo(".p4-anim-0",{opacity:0,y:20},{opacity:1,y:0,duration:.7,delay:.1,ease:"power3.out"}),w.fromTo(".p4-anim-1",{opacity:0,y:40},{opacity:1,y:0,duration:.9,delay:.25,ease:"power3.out"}),w.fromTo(".p4-anim-2",{opacity:0,y:30},{opacity:.5,y:0,duration:.8,delay:.4,ease:"power3.out"}),w.fromTo(".p4-anim-3",{opacity:0,y:20},{opacity:1,y:0,duration:.8,delay:.55,ease:"power3.out"}),w.fromTo(".p4-anim-4",{opacity:0,y:20},{opacity:1,y:0,duration:.8,delay:.7,ease:"power3.out"}),f(".p4-contrast-card",{y:40}),f("#p4-typo-section .p4-section-header",{y:40}),R(".p4-typo-controls"),R(".p4-typo-preview"),f("#p4-font-section .p4-section-header",{y:40}),f(".p4-font-compare",{y:30}),f(".p4-scale-table-wrap",{y:30}),f(".p4-lh-compare",{y:30}),f(".p4-journal-table-wrap",{y:30}),f("#p4-check-section .p4-section-header",{y:40}),w.utils.toArray(".p4-check-card").forEach((t,d)=>{f(t,{y:30})}),f("#p4-footer",{y:30}),document.querySelectorAll("#p4-hero-nav .hero-quicknav__item").forEach(t=>{t.addEventListener("click",()=>{const d=document.querySelector(t.dataset.target);d&&d.scrollIntoView({behavior:"smooth",block:"start"})})});try{U()}catch(t){console.error("p04 setupContrastChecker:",t)}try{J()}catch(t){console.error("p04 setupChartTypography:",t)}try{V()}catch(t){console.error("p04 setupNavButtons:",t)}}function U(){const t=document.getElementById("p4-fg-picker"),d=document.getElementById("p4-bg-picker"),b=document.getElementById("p4-fg-hex"),C=document.getElementById("p4-bg-hex"),z=document.getElementById("p4-fg-swatch"),u=document.getElementById("p4-bg-swatch");function y(){const r=N(a.fgColor,a.bgColor),v=r.toFixed(2);document.getElementById("p4-ratio-num").textContent=v;const m=r>=4.5,l=r>=3,h=r>=7;k("p4-badge-aa",m,"AA 正文"),k("p4-badge-aa-large",l,"AA 大字"),k("p4-badge-aaa",h,"AAA");const o=Math.min(100,r/21*100),g=document.getElementById("p4-bar-fill");g&&(g.style.width=o+"%",g.style.background=r>=4.5?"var(--color-success)":r>=3?"#F0B27A":"#ff3b30");const S=document.getElementById("p4-preview-box");S&&(S.style.background=a.bgColor,S.style.color=a.fgColor),z&&(z.style.background=a.fgColor),u&&(u.style.background=a.bgColor)}function k(r,v,m){const l=document.getElementById(r);l&&(l.className=`p4-badge ${v?"p4-badge--pass":"p4-badge--fail"}`,l.textContent=`${v?"✓":"✗"} ${m}`)}function A(r,v,m,l){!r||!v||(r.addEventListener("input",h=>{const o=h.target.value;l?a.fgColor=o:a.bgColor=o,v.value=o.toUpperCase(),y()}),v.addEventListener("input",h=>{let o=h.target.value.trim();o.startsWith("#")||(o="#"+o),/^#[0-9A-Fa-f]{6}$/.test(o)&&(l?a.fgColor=o:a.bgColor=o,r.value=o,y())}))}A(t,b,!1,!0),A(d,C,!1,!1);const n=document.getElementById("p4-font-dec"),c=document.getElementById("p4-font-inc"),p=document.getElementById("p4-font-size-val"),e=document.getElementById("p4-preview-normal");function i(){p&&(p.textContent=a.previewFontSize+"px"),e&&(e.style.fontSize=a.previewFontSize+"px")}n&&n.addEventListener("click",()=>{a.previewFontSize=Math.max(8,a.previewFontSize-1),i()}),c&&c.addEventListener("click",()=>{a.previewFontSize=Math.min(32,a.previewFontSize+1),i()}),y()}function J(){const t=document.getElementById("p4-chart-canvas");if(!t)return;const d=[{group:"Control",a:23,b:18,c:15},{group:"Treatment A",a:35,b:28,c:22},{group:"Treatment B",a:41,b:32,c:27},{group:"Treatment C",a:29,b:24,c:19}],b=["a","b","c"],C=["Gene α","Gene β","Gene γ"],z=["#7EC8E3","#5BA3C9","#3A7CA5"];function u(){t.innerHTML="";const n=t.getBoundingClientRect(),c=n.width||600,p=n.height||450,e=a.chart,i={top:20+e.titleSize+e.subtitleSize+8,right:100,bottom:20+e.axisLabelSize+e.tickSize+8,left:20+e.axisLabelSize+e.tickSize+8},r=W(t).append("svg").attr("viewBox",`0 0 ${c} ${p}`).attr("preserveAspectRatio","xMidYMid meet");r.append("rect").attr("width",c).attr("height",p).attr("fill","#ffffff");const v=c-i.left-i.right,m=p-i.top-i.bottom,l=r.append("g").attr("transform",`translate(${i.left},${i.top})`),h=j().domain(d.map(s=>s.group)).range([0,v]).padding(.25),o=j().domain(b).range([0,h.bandwidth()]).padding(.05),g=q().domain([0,50]).range([m,0]);l.selectAll(".grid-line").data(g.ticks(5)).enter().append("line").attr("x1",0).attr("x2",v).attr("y1",s=>g(s)).attr("y2",s=>g(s)).attr("stroke","#e8e8e8").attr("stroke-dasharray","3,3"),l.selectAll(".bar-group").data(d).enter().append("g").attr("transform",s=>`translate(${h(s.group)},0)`).selectAll("rect").data(s=>b.map(x=>({key:x,value:s[x]}))).enter().append("rect").attr("x",s=>o(s.key)).attr("y",s=>g(s.value)).attr("width",o.bandwidth()).attr("height",s=>m-g(s.value)).attr("fill",(s,x)=>z[x]).attr("rx",2),l.append("g").attr("transform",`translate(0,${m})`).call(G(h).tickSize(0)).selectAll("text").attr("font-size",e.tickSize+"px").attr("fill","#555"),l.select(".domain").attr("stroke","#ccc"),l.append("g").call(O(g).ticks(5).tickSize(-4)).selectAll("text").attr("font-size",e.tickSize+"px").attr("fill","#555"),r.append("text").attr("x",i.left+v/2).attr("y",p-6).attr("text-anchor","middle").attr("font-size",e.axisLabelSize+"px").attr("font-weight",600).attr("fill","#333").text("实验组"),r.append("text").attr("x",-(i.top+m/2)).attr("y",14).attr("transform","rotate(-90)").attr("text-anchor","middle").attr("font-size",e.axisLabelSize+"px").attr("font-weight",600).attr("fill","#333").text("表达量 (FPKM)"),r.append("text").attr("x",i.left).attr("y",e.titleSize+4).attr("font-size",e.titleSize+"px").attr("font-weight",e.titleWeight).attr("fill","#1d1d1f").text("三组基因在不同处理下的表达量"),r.append("text").attr("x",i.left).attr("y",e.titleSize+e.subtitleSize+10).attr("font-size",e.subtitleSize+"px").attr("fill","#888").text("数据来源：RNA-seq, n = 3 replicates");const L=i.top+10,M=c-i.right+16,I=r.append("g").attr("transform",`translate(${M},${L})`);b.forEach((s,x)=>{const F=x*(e.legendSize+10);I.append("rect").attr("x",0).attr("y",F).attr("width",e.legendSize).attr("height",e.legendSize).attr("fill",z[x]).attr("rx",2),I.append("text").attr("x",e.legendSize+6).attr("y",F+e.legendSize*.8).attr("font-size",e.legendSize+"px").attr("fill","#555").text(C[x])});const $=d[2],T=h($.group)+o("a")+o.bandwidth()/2,_=g($.a)-8;r.append("text").attr("x",i.left+T).attr("y",i.top+_).attr("text-anchor","middle").attr("font-size",e.annotationSize+"px").attr("font-style","italic").attr("fill","#888").text("p < 0.01 *")}const y=[{id:"p4-c-title",valId:"p4-v-title",key:"titleSize",unit:"px"},{id:"p4-c-subtitle",valId:"p4-v-subtitle",key:"subtitleSize",unit:"px"},{id:"p4-c-axis",valId:"p4-v-axis",key:"axisLabelSize",unit:"px"},{id:"p4-c-tick",valId:"p4-v-tick",key:"tickSize",unit:"px"},{id:"p4-c-legend",valId:"p4-v-legend",key:"legendSize",unit:"px"},{id:"p4-c-annotation",valId:"p4-v-annotation",key:"annotationSize",unit:"px"}];y.forEach(n=>{const c=document.getElementById(n.id),p=document.getElementById(n.valId);c&&c.addEventListener("input",e=>{const i=parseInt(e.target.value);a.chart[n.key]=i,p&&(p.textContent=i+n.unit),u()})});const k=document.getElementById("p4-typo-reset");k&&k.addEventListener("click",()=>{a.chart={...B},y.forEach(n=>{const c=document.getElementById(n.id),p=document.getElementById(n.valId);c&&(c.value=a.chart[n.key]),p&&(p.textContent=a.chart[n.key]+n.unit)}),u()}),u();const A=new ResizeObserver(()=>u());A.observe(t),a._resizeObserver=A}function V(){const t=document.getElementById("p4-next-btn"),d=document.getElementById("p4-prev-btn"),b=document.getElementById("p4-m1-btn");t&&t.addEventListener("click",()=>E("m1-p5")),d&&d.addEventListener("click",()=>E("m1-p3")),b&&b.addEventListener("click",()=>E("m1"))}function rt(){H(),a._resizeObserver&&(a._resizeObserver.disconnect(),a._resizeObserver=null),a={fgColor:"#1d1d1f",bgColor:"#ffffff",previewFontSize:16,chart:{...B},lineHeightA:1.2,lineHeightB:1.8}}export{rt as destroy,at as init,et as render};
