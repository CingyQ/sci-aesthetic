...
</svg>`},Ct={scatter:{size:3,alpha:.7,jitter:"none",regression:!1,shape:"circle"},bar:{arrangement:"grouped",barWidth:.7,errorBars:!1},line:{lineType:"solid",lineWidth:1.5,showPoints:!0,smooth:!1,fillArea:!1},boxplot:{boxWidth:.6,showOutliers:!0,fillAlpha:.7,notch:!1,showMean:!0,whisker:"iqr"},violin:{bw:.5,fillAlpha:.7,showBoxplot:!0,halfViolin:"full"},histogram:{bins:20,fillAlpha:.8,densityOverlay:!1},density:{bw:1,fillAlpha:.25,showRug:!1},heatmap:{colorScheme:"rdbu",showValues:!0},area:{arrangement:"stacked",fillAlpha:.65,showLines:!0},errorbar:{errorType:"se",capWidth:.2,lineWidth:1.5,dotSize:5},lollipop:{dotSize:8,lineWidth:1.2,sortBy:"none",horizontal:!1},ridgeline:{overlap:.7,fillAlpha:.7,bw:1}};let w={currentChart:"scatter",params:JSON.parse(JSON.stringify(Ct)),editor:null,copyBtn:null,cleanups:[]};function $(t){t.select(".domain").attr("stroke","#424245"),t.selectAll(".tick line").attr("stroke","#424245"),t.selectAll("text").attr("fill","#a1a1a6").style("font-size","11px")}function W(t){t.select(".domain").remove(),t.selectAll(".tick line").attr("stroke","rgba(255,255,255,0.05)"),t.selectAll("text").remove()}function A(t,e,a,o,s){a?t.append("text").attr("transform","rotate(-90)").attr("x",-s/2).attr("y",-48).attr("fill","#a1a1a6").attr("text-anchor","middle").style("font-size","12px").text(e):t.append("text").attr("x",o/2).attr("y",s+46).attr("fill","#a1a1a6").attr("text-anchor","middle").style("font-size","12px").text(e)}function gt(t){return e=>Math.exp(-.5*(e/t)**2)/(t*Math.sqrt(2*Math.PI))}function mt(t,e,a){return a.map(o=>({x:o,y:dt(t,s=>e(o-s))}))}function T(t,e,a,o){At(t).selectAll("*").remove();const s=At(t).append("svg").attr("viewBox",`0 0 ${e} ${a}`).attr("preserveAspectRatio","xMidYMid meet").style("width","100%").style("display","block");s.append("rect").attr("width",e).attr("height",a).attr("fill",kt).attr("rx",12);const r=s.append("g").attr("transform",`translate(${o.left},${o.top})`),i=e-o.left-o.right,h=a-o.top-o.bottom;return{svg:s,g:r,iW:i,iH:h}}function ke(t,e){const s={top:25,right:115,bottom:55,left:55},{g:r,iW:i,iH:h}=T(t,540,380,s),y=L().domain([3.8,8.2]).range([0,i]),b=L().domain([.5,8]).range([h,0]);r.append("g").call(E(b).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(y).ticks(6)).call($),r.append("g").call(E(b).ticks(5)).call($),A(r,"Sepal.Length (cm)",!1,i,h),A(r,"Petal.Length (cm)",!0,i,h),e.regression&&Object.entries(xt).forEach(([x,u])=>{const p=Wt.filter(C=>C.sp===x),m=dt(p,C=>C.x),v=dt(p,C=>C.y),k=bt(p,C=>(C.x-m)*(C.y-v))/bt(p,C=>(C.x-m)**2),P=v-k*m,j=D(p,C=>C.x)-.1,q=B(p,C=>C.x)+.1;r.append("line").attr("x1",y(j)).attr("y1",b(k*j+P)).attr("x2",y(q)).attr("y2",b(k*q+P)).attr("stroke",u).attr("stroke-width",1.5).attr("stroke-dasharray","6,4").attr("opacity",.7)});const d=z(999),g=e.jitter==="light"?.12:e.jitter==="heavy"?.28:0,f={circle:vt,triangle:ue,square:ye,cross:fe}[e.shape]||vt,l=(e.size+2)**2*6;Wt.forEach(x=>{const u=g>0?(d()-.5)*g:0,p=g>0?(d()-.5)*g:0;r.append("path").attr("d",_t().type(f).size(l)()).attr("transform",`translate(${y(x.x+u)},${b(x.y+p)})`).attr("fill",xt[x.sp]).attr("opacity",e.alpha).attr("stroke","none")});const n=r.append("g").attr("transform",`translate(${i+12},0)`);Object.entries(xt).forEach(([x,u],p)=>{const m=n.append("g").attr("transform",`translate(0,${p*22})`);m.append("path").attr("d",_t().type(f).size(64)()).attr("transform","translate(7,7)").attr("fill",u).attr("opacity",e.alpha),m.append("text").attr("x",18).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(x)})}function Ce(t,e){const s={top:25,right:115,bottom:60,left:55},{g:r,iW:i,iH:h}=T(t,540,380,s),y=be,b=y.map(f=>f.cat),d=["Week 1","Week 8"],g=R().domain(b).range([0,i]).paddingInner(1-e.barWidth);if(e.arrangement==="grouped"){const f=R().domain(d).range([0,g.bandwidth()]).padding(.08),l=B(y,x=>B(x.values,u=>u.mean+(e.errorBars?u.se:0))),n=L().domain([0,l*1.12]).range([h,0]);r.append("g").call(E(n).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(g)).call($),r.append("g").call(E(n).ticks(5)).call($),A(r,"Mean Value",!0,i,h),y.forEach(x=>{const u=r.append("g").attr("transform",`translate(${g(x.cat)},0)`);x.values.forEach((p,m)=>{if(u.append("rect").attr("x",f(p.grp)).attr("y",n(p.mean)).attr("width",f.bandwidth()).attr("height",h-n(p.mean)).attr("fill",st[m]).attr("rx",3),e.errorBars){const v=f(p.grp)+f.bandwidth()/2;u.append("line").attr("x1",v).attr("x2",v).attr("y1",n(p.mean+p.se)).attr("y2",n(p.mean-p.se)).attr("stroke","#fff").attr("stroke-width",1.5).attr("opacity",.7),[-p.se,p.se].forEach(k=>{u.append("line").attr("x1",v-4).attr("x2",v+4).attr("y1",n(p.mean+k)).attr("y2",n(p.mean+k)).attr("stroke","#fff").attr("stroke-width",1.5).attr("opacity",.7)})}})})}else if(e.arrangement==="stacked"){const f=y.map(u=>{const p={cat:u.cat};return u.values.forEach(m=>{p[m.grp]=m.mean}),p}),l=pt().keys(d)(f),n=B(l[l.length-1],u=>u[1]),x=L().domain([0,n*1.1]).range([h,0]);r.append("g").call(E(x).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(g)).call($),r.append("g").call(E(x).ticks(5)).call($),A(r,"Total Value",!0,i,h),l.forEach((u,p)=>{r.selectAll(null).data(u).enter().append("rect").attr("x",m=>g(m.data.cat)).attr("y",m=>x(m[1])).attr("width",g.bandwidth()).attr("height",m=>x(m[0])-x(m[1])).attr("fill",st[p]).attr("rx",p===0?3:0)})}else{const f=y.map(x=>{const u=bt(x.values,m=>m.mean),p={cat:x.cat};return x.values.forEach(m=>{p[m.grp]=m.mean/u*100}),p}),l=pt().keys(d)(f),n=L().domain([0,100]).range([h,0]);r.append("g").call(E(n).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").call(E(n).ticks(5).tickFormat(x=>x+"%")).call($),r.append("g").attr("transform",`translate(0,${h})`).call(_(g)).call($),A(r,"Proportion (%)",!0,i,h),l.forEach((x,u)=>{r.selectAll(null).data(x).enter().append("rect").attr("x",p=>g(p.data.cat)).attr("y",p=>n(p[1])).attr("width",g.bandwidth()).attr("height",p=>n(p[0])-n(p[1])).attr("fill",st[u]).attr("rx",u===0?3:0)})}const c=r.append("g").attr("transform",`translate(${i+12},0)`);d.forEach((f,l)=>{const n=c.append("g").attr("transform",`translate(0,${l*22})`);n.append("rect").attr("width",12).attr("height",12).attr("fill",st[l]).attr("rx",2),n.append("text").attr("x",16).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(f)})}function Ee(t,e){const s={top:25,right:110,bottom:55,left:55},{g:r,iW:i,iH:h}=T(t,540,380,s),y=["W0","W1","W2","W3","W4"],b=Ft().domain(y).range([0,i]).padding(.2),d=V.flatMap(n=>n.pts.map(x=>x.val)),g=L().domain([D(d)-8,B(d)+8]).range([h,0]);r.append("g").call(E(g).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(b)).call($),r.append("g").call(E(g).ticks(5)).call($),A(r,"Week",!1,i,h),A(r,"Response",!0,i,h);const c={solid:null,dashed:"8,4",dotted:"3,4"},f=e.smooth?Rt:pe;V.forEach(n=>{e.fillArea&&r.append("path").datum(n.pts).attr("d",J().x((u,p)=>b(y[p])).y0(h).y1(u=>g(u.val)).curve(f)).attr("fill",n.color).attr("opacity",.12);const x=r.append("path").datum(n.pts).attr("d",X().x((u,p)=>b(y[p])).y(u=>g(u.val)).curve(f)).attr("fill","none").attr("stroke",n.color).attr("stroke-width",e.lineWidth);c[e.lineType]&&x.attr("stroke-dasharray",c[e.lineType]),e.showPoints&&n.pts.forEach((u,p)=>{r.append("circle").attr("cx",b(y[p])).attr("cy",g(u.val)).attr("r",e.lineWidth+2).attr("fill",n.color).attr("stroke",kt).attr("stroke-width",1.5)})});const l=r.append("g").attr("transform",`translate(${i+12},0)`);V.forEach((n,x)=>{const u=l.append("g").attr("transform",`translate(0,${x*22})`);u.append("line").attr("x1",0).attr("x2",14).attr("y1",7).attr("y2",7).attr("stroke",n.color).attr("stroke-width",e.lineWidth),e.showPoints&&u.append("circle").attr("cx",7).attr("cy",7).attr("r",3).attr("fill",n.color),u.append("text").attr("x",18).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(n.grp)})}function $e(t,e){const s={top:25,right:30,bottom:50,left:60},{g:r,iW:i,iH:h}=T(t,540,380,s),y=St.map(c=>{const f=[...c.pts].sort((v,k)=>v-k),l=O(f,.25),n=O(f,.5),x=O(f,.75),u=x-l,p=e.whisker==="iqr"?Math.max(D(f),l-1.5*u):D(f),m=e.whisker==="iqr"?Math.min(B(f),x+1.5*u):B(f);return{name:c.name,q1:l,med:n,q3:x,iqr:u,wMin:p,wMax:m,outliers:f.filter(v=>v<p||v>m),mean:dt(f),notchLo:n-1.58*u/Math.sqrt(f.length),notchHi:n+1.58*u/Math.sqrt(f.length)}}),b=y.flatMap(c=>[c.wMin,c.wMax,...e.showOutliers?c.outliers:[]]),d=L().domain([D(b)-5,B(b)+5]).range([h,0]),g=R().domain(St.map(c=>c.name)).range([0,i]).padding(1-e.boxWidth);r.append("g").call(E(d).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(g)).call($),r.append("g").call(E(d).ticks(5)).call($),A(r,"Response Value",!0,i,h),y.forEach((c,f)=>{const l=g(c.name),n=g.bandwidth(),x=l+n/2,u=qt[f];if(r.append("line").attr("x1",x).attr("x2",x).attr("y1",d(c.wMax)).attr("y2",d(c.wMin)).attr("stroke",u).attr("stroke-width",1.5).attr("opacity",.7),[c.wMin,c.wMax].forEach(p=>{r.append("line").attr("x1",x-n*.2).attr("x2",x+n*.2).attr("y1",d(p)).attr("y2",d(p)).attr("stroke",u).attr("stroke-width",1.5)}),e.notch){const p=Math.max(c.q1,c.notchLo),m=Math.min(c.q3,c.notchHi),v=n*.2;r.append("path").attr("d",[`M ${l} ${d(c.q1)}`,`L ${l+n} ${d(c.q1)}`,`L ${l+n} ${d(p)}`,`L ${l+n-v} ${d(c.med)}`,`L ${l+n} ${d(m)}`,`L ${l+n} ${d(c.q3)}`,`L ${l} ${d(c.q3)}`,`L ${l} ${d(m)}`,`L ${l+v} ${d(c.med)}`,`L ${l} ${d(p)}`,"Z"].join(" ")).attr("fill",u).attr("opacity",e.fillAlpha).attr("stroke",u).attr("stroke-width",1.5)}else r.append("rect").attr("x",l).attr("y",d(c.q3)).attr("width",n).attr("height",d(c.q1)-d(c.q3)).attr("fill",u).attr("opacity",e.fillAlpha).attr("stroke",u).attr("stroke-width",1.5).attr("rx",3);if(r.append("line").attr("x1",l).attr("x2",l+n).attr("y1",d(c.med)).attr("y2",d(c.med)).attr("stroke","#fff").attr("stroke-width",2),e.showMean){const m=d(c.mean);r.append("path").attr("d",`M ${x} ${m-5} L ${x+5} ${m} L ${x} ${m+5} L ${x-5} ${m} Z`).attr("fill","#fff").attr("opacity",.9)}e.showOutliers&&c.outliers.forEach(p=>{r.append("circle").attr("cx",x).attr("cy",d(p)).attr("r",3).attr("fill","none").attr("stroke",u).attr("stroke-width",1.5).attr("opacity",.7)})})}function Be(t,e){const s={top:25,right:20,bottom:50,left:60},{g:r,iW:i,iH:h}=T(t,540,380,s),y=rt.flatMap(m=>m.pts),b=D(y)-5,d=B(y)+5,g=ht(b,d,(d-b)/80),c=gt(e.bw*10),f=R().domain(rt.map(m=>m.name)).range([0,i]).padding(.3),l=L().domain([b,d]).range([h,0]),n=f.bandwidth()/2,x=rt.map(m=>mt(m.pts,c,g)),u=B(x,m=>B(m,v=>v.y)),p=L().domain([0,u]).range([0,n*.92]);r.append("g").call(E(l).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(f)).call($),r.append("g").call(E(l).ticks(5)).call($),A(r,"Response Value",!0,i,h),rt.forEach((m,v)=>{const k=f(m.name)+n,P=qt[v],j=x[v],q=J().x0(C=>k-(e.halfViolin!=="right"?p(C.y):0)).x1(C=>k+(e.halfViolin!=="left"?p(C.y):0)).y(C=>l(C.x)).curve(Rt);if(r.append("path").datum(j).attr("d",q).attr("fill",P).attr("opacity",e.fillAlpha).attr("stroke",P).attr("stroke-width",1.5),e.showBoxplot){const C=[...m.pts].sort((Nt,Yt)=>Nt-Yt),I=O(C,.25),F=O(C,.5),Q=O(C,.75),Et=Q-I,Ot=Math.max(D(C),I-1.5*Et),Gt=Math.min(B(C),Q+1.5*Et),tt=n*.22;r.append("line").attr("x1",k).attr("x2",k).attr("y1",l(Ot)).attr("y2",l(Gt)).attr("stroke","#fff").attr("stroke-width",1.5).attr("opacity",.8),r.append("rect").attr("x",k-tt).attr("y",l(Q)).attr("width",tt*2).attr("height",l(I)-l(Q)).attr("fill","#111318").attr("stroke","#fff").attr("stroke-width",1.5).attr("rx",2),r.append("line").attr("x1",k-tt).attr("x2",k+tt).attr("y1",l(F)).attr("y2",l(F)).attr("stroke","#fff").attr("stroke-width",2.5)}})}function Ae(t,e){const s={top:25,right:30,bottom:55,left:55},{g:r,iW:i,iH:h}=T(t,540,380,s),y=D(ot)-2,b=B(ot)+2,d=L().domain([y,b]).range([0,i]),g=ae().domain([y,b]).thresholds(e.bins)(ot),c=B(g,l=>l.length),f=L().domain([0,c*1.12]).range([h,0]);if(r.append("g").call(E(f).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(d).ticks(8)).call($),r.append("g").call(E(f).ticks(5)).call($),A(r,"Value",!1,i,h),A(r,"Count",!0,i,h),r.selectAll(".p7-hbar").data(g).enter().append("rect").attr("class","p7-hbar").attr("x",l=>d(l.x0)+1).attr("y",l=>f(l.length)).attr("width",l=>Math.max(0,d(l.x1)-d(l.x0)-1)).attr("height",l=>h-f(l.length)).attr("fill","#7EC8E3").attr("opacity",e.fillAlpha).attr("rx",2),e.densityOverlay){const l=gt(3.2),n=ht(y,b,(b-y)/100),x=mt(ot,l,n),u=B(x,m=>m.y),p=c/u;r.append("path").datum(x).attr("d",X().x(m=>d(m.x)).y(m=>f(m.y*p)).curve(Y)).attr("fill","none").attr("stroke","#F0B27A").attr("stroke-width",2.5).attr("opacity",.9),r.append("text").attr("x",i-4).attr("y",14).attr("fill","#F0B27A").style("font-size","10px").attr("text-anchor","end").text("密度曲线")}}function Le(t,e){const s={top:25,right:105,bottom:55,left:60},{g:r,iW:i,iH:h}=T(t,540,380,s),y=ut.flatMap(p=>p.pts),b=D(y)-5,d=B(y)+5,g=L().domain([b,d]).range([0,i]),c=gt(e.bw*5),f=ht(b,d,(d-b)/100),l=ut.map(p=>({...p,kde:mt(p.pts,c,f)})),n=B(l,p=>B(p.kde,m=>m.y)),x=L().domain([0,n*1.12]).range([h,0]);r.append("g").call(E(x).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(g).ticks(6)).call($),r.append("g").call(E(x).ticks(4).tickFormat(wt(".3f"))).call($),A(r,"Value",!1,i,h),A(r,"Density",!0,i,h),l.forEach(p=>{r.append("path").datum(p.kde).attr("d",J().x(m=>g(m.x)).y0(h).y1(m=>x(m.y)).curve(Y)).attr("fill",p.color).attr("opacity",e.fillAlpha),r.append("path").datum(p.kde).attr("d",X().x(m=>g(m.x)).y(m=>x(m.y)).curve(Y)).attr("fill","none").attr("stroke",p.color).attr("stroke-width",2),e.showRug&&p.pts.forEach(m=>{r.append("line").attr("x1",g(m)).attr("x2",g(m)).attr("y1",h).attr("y2",h-6).attr("stroke",p.color).attr("opacity",.4)})});const u=r.append("g").attr("transform",`translate(${i+12},0)`);ut.forEach((p,m)=>{const v=u.append("g").attr("transform",`translate(0,${m*22})`);v.append("rect").attr("width",12).attr("height",3).attr("y",5).attr("fill",p.color).attr("rx",1),v.append("text").attr("x",16).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(p.grp)})}function Me(t,e){const s={top:20,right:80,bottom:60,left:70},{g:r,iW:i,iH:h}=T(t,540,380,s),y=it.map(v=>v.gene),b=it[0].values.map(v=>v.cond),d=R().domain(b).range([0,i]).padding(.04),g=R().domain(y).range([0,h]).padding(.04),c=it.flatMap(v=>v.values.map(k=>k.val)),f=B(c.map(Math.abs)),l={rdbu:It(ce).domain([f,0,-f]),viridis:Lt(se).domain([-f,f]),plasma:Lt(le).domain([-f,f])},n=l[e.colorScheme]||l.rdbu;r.append("g").attr("transform",`translate(0,${h})`).call(_(d)).call(v=>{v.select(".domain").attr("stroke","#424245"),v.selectAll(".tick line").remove(),v.selectAll("text").attr("fill","#a1a1a6").style("font-size","10px")}),r.append("g").call(E(g)).call(v=>{v.select(".domain").attr("stroke","#424245"),v.selectAll(".tick line").remove(),v.selectAll("text").attr("fill","#a1a1a6").style("font-size","10px").attr("font-style","italic")}),it.forEach(v=>{v.values.forEach(k=>{r.append("rect").attr("x",d(k.cond)).attr("y",g(v.gene)).attr("width",d.bandwidth()).attr("height",g.bandwidth()).attr("fill",n(k.val)).attr("rx",3),e.showValues&&r.append("text").attr("x",d(k.cond)+d.bandwidth()/2).attr("y",g(v.gene)+g.bandwidth()/2+4).attr("text-anchor","middle").attr("fill",Math.abs(k.val)>f*.5?"#fff":"#444").style("font-size","9px").style("font-family","monospace").text(k.val.toFixed(1))})});const x=h,u=14,p=r.append("defs").append("linearGradient").attr("id","p7-hm-grad").attr("x1",0).attr("y1",1).attr("x2",0).attr("y2",0);[0,.25,.5,.75,1].forEach(v=>{p.append("stop").attr("offset",`${v*100}%`).attr("stop-color",n(-f+v*2*f))}),r.append("rect").attr("x",i+20).attr("y",0).attr("width",u).attr("height",x).attr("fill","url(#p7-hm-grad)").attr("rx",3);const m=L().domain([-f,f]).range([x,0]);r.append("g").attr("transform",`translate(${i+20+u},0)`).call(ne(m).ticks(4).tickFormat(wt(".1f"))).call(v=>{v.select(".domain").remove(),v.selectAll(".tick line").attr("stroke","#424245"),v.selectAll("text").attr("fill","#a1a1a6").style("font-size","9px")})}function _e(t,e){const s={top:25,right:110,bottom:55,left:55},{g:r,iW:i,iH:h}=T(t,540,380,s),y=["W0","W1","W2","W3","W4"],b=Ft().domain(y).range([0,i]).padding(.2),d=y.map((u,p)=>{const m={w:u};return V.forEach(v=>{m[v.grp]=v.pts[p].val}),m}),g=V.map(u=>u.grp),c=V.map(u=>u.color),f=(u,p)=>J().x((m,v)=>b(y[v])).y0(m=>p(m[0])).y1(m=>p(m[1])).curve(Mt),l=(u,p)=>X().x((m,v)=>b(y[v])).y(m=>p(m[1])).curve(Mt);let n;if(e.arrangement==="proportional"){const u=pt().keys(g).offset(xe)(d);n=L().domain([0,1]).range([h,0]),r.append("g").call(E(n).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(b)).call($),r.append("g").call(E(n).ticks(5).tickFormat(wt(".0%"))).call($),A(r,"Week",!1,i,h),A(r,"Proportion",!0,i,h),u.forEach((p,m)=>{r.append("path").datum(p).attr("d",f(p,n)).attr("fill",c[m]).attr("opacity",e.fillAlpha),e.showLines&&r.append("path").datum(p).attr("d",l(p,n)).attr("fill","none").attr("stroke",c[m]).attr("stroke-width",1.5).attr("opacity",.8)})}else{const u=pt().keys(g)(d),p=B(u[u.length-1],m=>m[1]);n=L().domain([0,p*1.05]).range([h,0]),r.append("g").call(E(n).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(b)).call($),r.append("g").call(E(n).ticks(5)).call($),A(r,"Week",!1,i,h),A(r,"Response",!0,i,h),u.forEach((m,v)=>{r.append("path").datum(m).attr("d",f(m,n)).attr("fill",c[v]).attr("opacity",e.fillAlpha),e.showLines&&r.append("path").datum(m).attr("d",l(m,n)).attr("fill","none").attr("stroke",c[v]).attr("stroke-width",1.5).attr("opacity",.8)})}const x=r.append("g").attr("transform",`translate(${i+12},0)`);V.forEach((u,p)=>{const m=x.append("g").attr("transform",`translate(0,${p*22})`);m.append("rect").attr("width",12).attr("height",12).attr("fill",u.color).attr("opacity",e.fillAlpha).attr("rx",2),m.append("text").attr("x",16).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(u.grp)})}function We(t,e){const s={top:25,right:30,bottom:65,left:55},{g:r,iW:i,iH:h}=T(t,540,380,s),y=e.errorType==="se"?"se":e.errorType==="sd"?"sd":"ci95",b=B(nt,l=>l.mean+l[y]),d=Math.max(0,D(nt,l=>l.mean-l[y])),g=R().domain(nt.map(l=>l.grp)).range([0,i]).padding(.5),c=L().domain([d*.85,b*1.15]).range([h,0]);r.append("g").call(E(c).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${h})`).call(_(g)).call($).selectAll("text").attr("transform","rotate(-30)").attr("text-anchor","end").attr("dy","0.4em"),r.append("g").call(E(c).ticks(5)).call($),A(r,"Mean Value",!0,i,h),nt.forEach((l,n)=>{const x=g(l.grp)+g.bandwidth()/2,u=l[y],p=n<3?"#7EC8E3":"#F0B27A",m=e.capWidth*g.bandwidth();r.append("line").attr("x1",x).attr("x2",x).attr("y1",c(l.mean+u)).attr("y2",c(l.mean-u)).attr("stroke",p).attr("stroke-width",e.lineWidth),[-u,u].forEach(v=>{r.append("line").attr("x1",x-m).attr("x2",x+m).attr("y1",c(l.mean+v)).attr("y2",c(l.mean+v)).attr("stroke",p).attr("stroke-width",e.lineWidth)}),r.append("circle").attr("cx",x).attr("cy",c(l.mean)).attr("r",e.dotSize/2).attr("fill",p).attr("stroke",kt).attr("stroke-width",1.5)});const f={se:"Error bars: ±SE",sd:"Error bars: ±SD",ci95:"Error bars: 95% CI"};r.append("text").attr("x",i).attr("y",14).attr("text-anchor","end").attr("fill","#a1a1a6").style("font-size","10px").text(f[e.errorType])}function Se(t,e){const a=e.horizontal?55:100,o=540,s=380,r={top:25,right:30,bottom:a,left:e.horizontal?80:55},{g:i,iW:h,iH:y}=T(t,o,s,r);let b=[...ve];e.sortBy==="asc"&&b.sort((g,c)=>g.val-c.val),e.sortBy==="desc"&&b.sort((g,c)=>c.val-g.val);const d=B(b,g=>Math.abs(g.val))*1.15;if(e.horizontal){const g=R().domain(b.map(l=>l.gene)).range([0,y]).padding(.4),c=L().domain([-d,d]).range([0,h]),f=c(0);i.append("g").call(E(g).ticks(0).tickSize(-h).tickFormat("")).call(W),i.append("g").call(_(c).ticks(5).tickSize(-y).tickFormat("")).call(W),i.append("g").call(E(g)).call(l=>{l.select(".domain").attr("stroke","#424245"),l.selectAll(".tick line").remove(),l.selectAll("text").attr("fill","#a1a1a6").style("font-size","10px").attr("font-style","italic")}),i.append("g").attr("transform",`translate(0,${y})`).call(_(c).ticks(5)).call($),i.append("line").attr("x1",f).attr("x2",f).attr("y1",0).attr("y2",y).attr("stroke","rgba(255,255,255,0.15)").attr("stroke-width",1),A(i,"log2 Fold Change",!1,h,y),b.forEach(l=>{const n=g(l.gene)+g.bandwidth()/2,x=l.val>0?"#F0B27A":"#7EC8E3";i.append("line").attr("x1",f).attr("x2",c(l.val)).attr("y1",n).attr("y2",n).attr("stroke",x).attr("stroke-width",e.lineWidth).attr("opacity",.7),i.append("circle").attr("cx",c(l.val)).attr("cy",n).attr("r",e.dotSize/2).attr("fill",x)})}else{const g=R().domain(b.map(l=>l.gene)).range([0,h]).padding(.4),c=L().domain([-d,d]).range([y,0]),f=c(0);i.append("g").call(E(c).ticks(5).tickSize(-h).tickFormat("")).call(W),i.append("g").attr("transform",`translate(0,${y})`).call(_(g)).call($).selectAll("text").attr("transform","rotate(-35)").attr("text-anchor","end").attr("dy","0.4em").attr("font-style","italic"),i.append("g").call(E(c).ticks(5)).call($),i.append("line").attr("x1",0).attr("x2",h).attr("y1",f).attr("y2",f).attr("stroke","rgba(255,255,255,0.15)").attr("stroke-width",1),A(i,"Gene",!1,h,y),A(i,"log2 FC",!0,h,y),b.forEach(l=>{const n=g(l.gene)+g.bandwidth()/2,x=l.val>0?"#F0B27A":"#7EC8E3";i.append("line").attr("x1",n).attr("x2",n).attr("y1",f).attr("y2",c(l.val)).attr("stroke",x).attr("stroke-width",e.lineWidth).attr("opacity",.7),i.append("circle").attr("cx",n).attr("cy",c(l.val)).attr("r",e.dotSize/2).attr("fill",x)})}}function De(t,e){const s={top:30,right:20,bottom:50,left:90},{g:r,iW:i,iH:h}=T(t,540,380,s),y=lt.flatMap(v=>v.pts),b=D(y)-5,d=B(y)+5,g=L().domain([b,d]).range([0,i]),c=gt(e.bw*5),f=ht(b,d,(d-b)/100),l=lt.length,n=h/(l+.5),x=lt.map(v=>mt(v.pts,c,f)),u=B(x,v=>B(v,k=>k.y)),p=n*(1+e.overlap),m=["#7EC8E3","#F0B27A","#95D5B2","#B8B8E8"];r.append("g").attr("transform",`translate(0,${h})`).call(_(g).ticks(6)).call($),A(r,"Value",!1,i,h),lt.forEach((v,k)=>{const P=x[k],j=h-k*n,q=m[k%m.length],C=L().domain([0,u]).range([0,-p]),I=r.append("g").attr("transform",`translate(0,${j})`);I.append("path").datum(P).attr("d",J().x(F=>g(F.x)).y0(0).y1(F=>C(F.y)).curve(Y)).attr("fill",q).attr("opacity",e.fillAlpha),I.append("line").attr("x1",0).attr("x2",i).attr("y1",0).attr("y2",0).attr("stroke","rgba(255,255,255,0.1)").attr("stroke-width",1),I.append("path").datum(P).attr("d",X().x(F=>g(F.x)).y(F=>C(F.y)).curve(Y)).attr("fill","none").attr("stroke",q).attr("stroke-width",2),I.append("text").attr("x",-8).attr("y",0).attr("text-anchor","end").attr("fill","#a1a1a6").style("font-size","10px").attr("dominant-baseline","middle").text(v.tp)})}const ze={scatter:ke,bar:Ce,line:Ee,boxplot:$e,violin:Be,histogram:Ae,density:Le,heatmap:Me,area:_e,errorbar:We,lollipop:Se,ridgeline:De},Te={circle:"16",triangle:"17",square:"15",cross:"3"},Fe={solid:'"solid"',dashed:'"dashed"',dotted:'"dotted"'};function Re(t){const e=t.jitter!=="none"?`    position = position_jitter(
...
ggplot(data, aes(
  ...
  <input class="p7-slider" type="range" data-param="${t}"
    ...
    <div class="p7-ctrl-group">${M("bw",.5,5,.1,t.bw)}</div>
    <button class="p7-reset-btn" data-reset="true">↺ 重置参数</button>`}const la={scatter:Ue,bar:Ke,line:Je,boxplot:Xe,violin:Qe,histogram:ta,density:ea,heatmap:aa,area:ra,errorbar:oa,lollipop:ia,ridgeline:na};function Sa(){const t=`<svg class="p7-chevron" width="16" height="16" viewBox="0 0 24 24"
