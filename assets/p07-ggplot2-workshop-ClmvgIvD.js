...
</svg>`},$t={scatter:{size:3,alpha:.7,jitter:"none",regression:!1,shape:"circle"},bar:{arrangement:"grouped",barWidth:.7,errorBars:!1},line:{lineType:"solid",lineWidth:1.5,showPoints:!0,smooth:!1,fillArea:!1},boxplot:{boxWidth:.6,showOutliers:!0,fillAlpha:.7,notch:!1,showMean:!0,whisker:"iqr"},violin:{bw:.5,fillAlpha:.7,showBoxplot:!0,halfViolin:"full"},histogram:{bins:20,fillAlpha:.8,densityOverlay:!1},density:{bw:1,fillAlpha:.25,showRug:!1},heatmap:{colorScheme:"rdbu",showValues:!0},area:{arrangement:"stacked",fillAlpha:.65,showLines:!0},errorbar:{errorType:"se",capWidth:.2,lineWidth:1.5,dotSize:5},lollipop:{dotSize:8,lineWidth:1.2,sortBy:"none",horizontal:!1},ridgeline:{overlap:.7,fillAlpha:.7,bw:1}};let w={currentChart:"scatter",params:JSON.parse(JSON.stringify($t)),editor:null,copyBtn:null,cleanups:[]};function $(t){t.select(".domain").attr("stroke","#424245"),t.selectAll(".tick line").attr("stroke","#424245"),t.selectAll("text").attr("fill","#a1a1a6").style("font-size","11px")}function W(t){t.select(".domain").remove(),t.selectAll(".tick line").attr("stroke","rgba(255,255,255,0.05)"),t.selectAll("text").remove()}function A(t,e,a,o,s){a?t.append("text").attr("transform","rotate(-90)").attr("x",-s/2).attr("y",-48).attr("fill","#a1a1a6").attr("text-anchor","middle").style("font-size","12px").text(e):t.append("text").attr("x",o/2).attr("y",s+46).attr("fill","#a1a1a6").attr("text-anchor","middle").style("font-size","12px").text(e)}function mt(t){return e=>Math.exp(-.5*(e/t)**2)/(t*Math.sqrt(2*Math.PI))}function bt(t,e,a){return a.map(o=>({x:o,y:ft(t,s=>e(o-s))}))}function T(t,e,a,o){Lt(t).selectAll("*").remove();const s=Lt(t).append("svg").attr("viewBox",`0 0 ${e} ${a}`).attr("preserveAspectRatio","xMidYMid meet").style("width","100%").style("display","block");s.append("rect").attr("width",e).attr("height",a).attr("fill",Et).attr("rx",12);const r=s.append("g").attr("transform",`translate(${o.left},${o.top})`),i=e-o.left-o.right,f=a-o.top-o.bottom;return{svg:s,g:r,iW:i,iH:f}}function _e(t,e){const s={top:25,right:115,bottom:55,left:55},{g:r,iW:i,iH:f}=T(t,540,380,s),b=_().domain([3.8,8.2]).range([0,i]),y=_().domain([.5,8]).range([f,0]);r.append("g").call(E(y).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(b).ticks(6)).call($),r.append("g").call(E(y).ticks(5)).call($),A(r,"Sepal.Length (cm)",!1,i,f),A(r,"Petal.Length (cm)",!0,i,f),e.regression&&Object.entries(vt).forEach(([x,u])=>{const d=zt.filter(C=>C.sp===x),g=ft(d,C=>C.x),v=ft(d,C=>C.y),k=wt(d,C=>(C.x-g)*(C.y-v))/wt(d,C=>(C.x-g)**2),P=v-k*g,j=D(d,C=>C.x)-.1,q=B(d,C=>C.x)+.1;r.append("line").attr("x1",b(j)).attr("y1",y(k*j+P)).attr("x2",b(q)).attr("y2",y(k*q+P)).attr("stroke",u).attr("stroke-width",1.5).attr("stroke-dasharray","6,4").attr("opacity",.7)});const p=z(999),h=e.jitter==="light"?.12:e.jitter==="heavy"?.28:0,m={circle:kt,triangle:Ce,square:ke,cross:we}[e.shape]||kt,l=(e.size+2)**2*6;zt.forEach(x=>{const u=h>0?(p()-.5)*h:0,d=h>0?(p()-.5)*h:0;r.append("path").attr("d",St().type(m).size(l)()).attr("transform",`translate(${b(x.x+u)},${y(x.y+d)})`).attr("fill",vt[x.sp]).attr("opacity",e.alpha).attr("stroke","none")});const n=r.append("g").attr("transform",`translate(${i+12},0)`);Object.entries(vt).forEach(([x,u],d)=>{const g=n.append("g").attr("transform",`translate(0,${d*22})`);g.append("path").attr("d",St().type(m).size(64)()).attr("transform","translate(7,7)").attr("fill",u).attr("opacity",e.alpha),g.append("text").attr("x",18).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(x)})}function Le(t,e){const s={top:25,right:115,bottom:60,left:55},{g:r,iW:i,iH:f}=T(t,540,380,s),b=$e,y=b.map(m=>m.cat),p=["Week 1","Week 8"],h=R().domain(y).range([0,i]).paddingInner(1-e.barWidth);if(e.arrangement==="grouped"){const m=R().domain(p).range([0,h.bandwidth()]).padding(.08),l=B(b,x=>B(x.values,u=>u.mean+(e.errorBars?u.se:0))),n=_().domain([0,l*1.12]).range([f,0]);r.append("g").call(E(n).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(h)).call($),r.append("g").call(E(n).ticks(5)).call($),A(r,"Mean Value",!0,i,f),b.forEach(x=>{const u=r.append("g").attr("transform",`translate(${h(x.cat)},0)`);x.values.forEach((d,g)=>{if(u.append("rect").attr("x",m(d.grp)).attr("y",n(d.mean)).attr("width",m.bandwidth()).attr("height",f-n(d.mean)).attr("fill",ct[g]).attr("rx",3),e.errorBars){const v=m(d.grp)+m.bandwidth()/2;u.append("line").attr("x1",v).attr("x2",v).attr("y1",n(d.mean+d.se)).attr("y2",n(d.mean-d.se)).attr("stroke","#fff").attr("stroke-width",1.5).attr("opacity",.7),[-d.se,d.se].forEach(k=>{u.append("line").attr("x1",v-4).attr("x2",v+4).attr("y1",n(d.mean+k)).attr("y2",n(d.mean+k)).attr("stroke","#fff").attr("stroke-width",1.5).attr("opacity",.7)})}})})}else if(e.arrangement==="stacked"){const m=b.map(u=>{const d={cat:u.cat};return u.values.forEach(g=>{d[g.grp]=g.mean}),d}),l=pt().keys(p)(m),n=B(l[l.length-1],u=>u[1]),x=_().domain([0,n*1.1]).range([f,0]);r.append("g").call(E(x).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(h)).call($),r.append("g").call(E(x).ticks(5)).call($),A(r,"Total Value",!0,i,f),l.forEach((u,d)=>{r.selectAll(null).data(u).enter().append("rect").attr("x",g=>h(g.data.cat)).attr("y",g=>x(g[1])).attr("width",h.bandwidth()).attr("height",g=>x(g[0])-x(g[1])).attr("fill",ct[d]).attr("rx",d===0?3:0)})}else{const m=b.map(x=>{const u=wt(x.values,g=>g.mean),d={cat:x.cat};return x.values.forEach(g=>{d[g.grp]=g.mean/u*100}),d}),l=pt().keys(p)(m),n=_().domain([0,100]).range([f,0]);r.append("g").call(E(n).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").call(E(n).ticks(5).tickFormat(x=>x+"%")).call($),r.append("g").attr("transform",`translate(0,${f})`).call(M(h)).call($),A(r,"Proportion (%)",!0,i,f),l.forEach((x,u)=>{r.selectAll(null).data(x).enter().append("rect").attr("x",d=>h(d.data.cat)).attr("y",d=>n(d[1])).attr("width",h.bandwidth()).attr("height",d=>n(d[0])-n(d[1])).attr("fill",ct[u]).attr("rx",u===0?3:0)})}const c=r.append("g").attr("transform",`translate(${i+12},0)`);p.forEach((m,l)=>{const n=c.append("g").attr("transform",`translate(0,${l*22})`);n.append("rect").attr("width",12).attr("height",12).attr("fill",ct[l]).attr("rx",2),n.append("text").attr("x",16).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(m)})}function Me(t,e){const s={top:25,right:110,bottom:55,left:55},{g:r,iW:i,iH:f}=T(t,540,380,s),b=["W0","W1","W2","W3","W4"],y=Pt().domain(b).range([0,i]).padding(.2),p=V.flatMap(n=>n.pts.map(x=>x.val)),h=_().domain([D(p)-8,B(p)+8]).range([f,0]);r.append("g").call(E(h).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(y)).call($),r.append("g").call(E(h).ticks(5)).call($),A(r,"Week",!1,i,f),A(r,"Response",!0,i,f);const c={solid:null,dashed:"8,4",dotted:"3,4"},m=e.smooth?It:fe;V.forEach(n=>{e.fillArea&&r.append("path").datum(n.pts).attr("d",X().x((u,d)=>y(b[d])).y0(f).y1(u=>h(u.val)).curve(m)).attr("fill",n.color).attr("opacity",.12);const x=r.append("path").datum(n.pts).attr("d",Q().x((u,d)=>y(b[d])).y(u=>h(u.val)).curve(m)).attr("fill","none").attr("stroke",n.color).attr("stroke-width",e.lineWidth);c[e.lineType]&&x.attr("stroke-dasharray",c[e.lineType]),e.showPoints&&n.pts.forEach((u,d)=>{r.append("circle").attr("cx",y(b[d])).attr("cy",h(u.val)).attr("r",e.lineWidth+2).attr("fill",n.color).attr("stroke",Et).attr("stroke-width",1.5)})});const l=r.append("g").attr("transform",`translate(${i+12},0)`);V.forEach((n,x)=>{const u=l.append("g").attr("transform",`translate(0,${x*22})`);u.append("line").attr("x1",0).attr("x2",14).attr("y1",7).attr("y2",7).attr("stroke",n.color).attr("stroke-width",e.lineWidth),e.showPoints&&u.append("circle").attr("cx",7).attr("cy",7).attr("r",3).attr("fill",n.color),u.append("text").attr("x",18).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(n.grp)})}function We(t,e){const s={top:25,right:30,bottom:50,left:60},{g:r,iW:i,iH:f}=T(t,540,380,s),b=Tt.map(c=>{const m=[...c.pts].sort((v,k)=>v-k),l=O(m,.25),n=O(m,.5),x=O(m,.75),u=x-l,d=e.whisker==="iqr"?Math.max(D(m),l-1.5*u):D(m),g=e.whisker==="iqr"?Math.min(B(m),x+1.5*u):B(m);return{name:c.name,q1:l,med:n,q3:x,iqr:u,wMin:d,wMax:g,outliers:m.filter(v=>v<d||v>g),mean:ft(m),notchLo:n-1.58*u/Math.sqrt(m.length),notchHi:n+1.58*u/Math.sqrt(m.length)}}),y=b.flatMap(c=>[c.wMin,c.wMax,...e.showOutliers?c.outliers:[]]),p=_().domain([D(y)-5,B(y)+5]).range([f,0]),h=R().domain(Tt.map(c=>c.name)).range([0,i]).padding(1-e.boxWidth);r.append("g").call(E(p).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(h)).call($),r.append("g").call(E(p).ticks(5)).call($),A(r,"Response Value",!0,i,f),b.forEach((c,m)=>{const l=h(c.name),n=h.bandwidth(),x=l+n/2,u=Gt[m];if(r.append("line").attr("x1",x).attr("x2",x).attr("y1",p(c.wMax)).attr("y2",p(c.wMin)).attr("stroke",u).attr("stroke-width",1.5).attr("opacity",.7),[c.wMin,c.wMax].forEach(d=>{r.append("line").attr("x1",x-n*.2).attr("x2",x+n*.2).attr("y1",p(d)).attr("y2",p(d)).attr("stroke",u).attr("stroke-width",1.5)}),e.notch){const d=Math.max(c.q1,c.notchLo),g=Math.min(c.q3,c.notchHi),v=n*.2;r.append("path").attr("d",[`M ${l} ${p(c.q1)}`,`L ${l+n} ${p(c.q1)}`,`L ${l+n} ${p(d)}`,`L ${l+n-v} ${p(c.med)}`,`L ${l+n} ${p(g)}`,`L ${l+n} ${p(c.q3)}`,`L ${l} ${p(c.q3)}`,`L ${l} ${p(g)}`,`L ${l+v} ${p(c.med)}`,`L ${l} ${p(d)}`,"Z"].join(" ")).attr("fill",u).attr("opacity",e.fillAlpha).attr("stroke",u).attr("stroke-width",1.5)}else r.append("rect").attr("x",l).attr("y",p(c.q3)).attr("width",n).attr("height",p(c.q1)-p(c.q3)).attr("fill",u).attr("opacity",e.fillAlpha).attr("stroke",u).attr("stroke-width",1.5).attr("rx",3);if(r.append("line").attr("x1",l).attr("x2",l+n).attr("y1",p(c.med)).attr("y2",p(c.med)).attr("stroke","#fff").attr("stroke-width",2),e.showMean){const g=p(c.mean);r.append("path").attr("d",`M ${x} ${g-5} L ${x+5} ${g} L ${x} ${g+5} L ${x-5} ${g} Z`).attr("fill","#fff").attr("opacity",.9)}e.showOutliers&&c.outliers.forEach(d=>{r.append("circle").attr("cx",x).attr("cy",p(d)).attr("r",3).attr("fill","none").attr("stroke",u).attr("stroke-width",1.5).attr("opacity",.7)})})}function Se(t,e){const s={top:25,right:20,bottom:50,left:60},{g:r,iW:i,iH:f}=T(t,540,380,s),b=ot.flatMap(g=>g.pts),y=D(b)-5,p=B(b)+5,h=ht(y,p,(p-y)/80),c=mt(e.bw*10),m=R().domain(ot.map(g=>g.name)).range([0,i]).padding(.3),l=_().domain([y,p]).range([f,0]),n=m.bandwidth()/2,x=ot.map(g=>bt(g.pts,c,h)),u=B(x,g=>B(g,v=>v.y)),d=_().domain([0,u]).range([0,n*.92]);r.append("g").call(E(l).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(m)).call($),r.append("g").call(E(l).ticks(5)).call($),A(r,"Response Value",!0,i,f),ot.forEach((g,v)=>{const k=m(g.name)+n,P=Gt[v],j=x[v],q=X().x0(C=>k-(e.halfViolin!=="right"?d(C.y):0)).x1(C=>k+(e.halfViolin!=="left"?d(C.y):0)).y(C=>l(C.x)).curve(It);if(r.append("path").datum(j).attr("d",q).attr("fill",P).attr("opacity",e.fillAlpha).attr("stroke",P).attr("stroke-width",1.5),e.showBoxplot){const C=[...g.pts].sort((Kt,Jt)=>Kt-Jt),I=O(C,.25),F=O(C,.5),tt=O(C,.75),Bt=tt-I,Zt=Math.max(D(C),I-1.5*Bt),Ut=Math.min(B(C),tt+1.5*Bt),et=n*.22;r.append("line").attr("x1",k).attr("x2",k).attr("y1",l(Zt)).attr("y2",l(Ut)).attr("stroke","#fff").attr("stroke-width",1.5).attr("opacity",.8),r.append("rect").attr("x",k-et).attr("y",l(tt)).attr("width",et*2).attr("height",l(I)-l(tt)).attr("fill","#111318").attr("stroke","#fff").attr("stroke-width",1.5).attr("rx",2),r.append("line").attr("x1",k-et).attr("x2",k+et).attr("y1",l(F)).attr("y2",l(F)).attr("stroke","#fff").attr("stroke-width",2.5)}})}function De(t,e){const s={top:25,right:30,bottom:55,left:55},{g:r,iW:i,iH:f}=T(t,540,380,s),b=D(it)-2,y=B(it)+2,p=_().domain([b,y]).range([0,i]),h=se().domain([b,y]).thresholds(e.bins)(it),c=B(h,l=>l.length),m=_().domain([0,c*1.12]).range([f,0]);if(r.append("g").call(E(m).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(p).ticks(8)).call($),r.append("g").call(E(m).ticks(5)).call($),A(r,"Value",!1,i,f),A(r,"Count",!0,i,f),r.selectAll(".p7-hbar").data(h).enter().append("rect").attr("class","p7-hbar").attr("x",l=>p(l.x0)+1).attr("y",l=>m(l.length)).attr("width",l=>Math.max(0,p(l.x1)-p(l.x0)-1)).attr("height",l=>f-m(l.length)).attr("fill","#7EC8E3").attr("opacity",e.fillAlpha).attr("rx",2),e.densityOverlay){const l=mt(3.2),n=ht(b,y,(y-b)/100),x=bt(it,l,n),u=B(x,g=>g.y),d=c/u;r.append("path").datum(x).attr("d",Q().x(g=>p(g.x)).y(g=>m(g.y*d)).curve(Y)).attr("fill","none").attr("stroke","#F0B27A").attr("stroke-width",2.5).attr("opacity",.9),r.append("text").attr("x",i-4).attr("y",14).attr("fill","#F0B27A").style("font-size","10px").attr("text-anchor","end").text("密度曲线")}}function ze(t,e){const s={top:25,right:105,bottom:55,left:60},{g:r,iW:i,iH:f}=T(t,540,380,s),b=yt.flatMap(d=>d.pts),y=D(b)-5,p=B(b)+5,h=_().domain([y,p]).range([0,i]),c=mt(e.bw*5),m=ht(y,p,(p-y)/100),l=yt.map(d=>({...d,kde:bt(d.pts,c,m)})),n=B(l,d=>B(d.kde,g=>g.y)),x=_().domain([0,n*1.12]).range([f,0]);r.append("g").call(E(x).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(h).ticks(6)).call($),r.append("g").call(E(x).ticks(4).tickFormat(Ct(".3f"))).call($),A(r,"Value",!1,i,f),A(r,"Density",!0,i,f),l.forEach(d=>{r.append("path").datum(d.kde).attr("d",X().x(g=>h(g.x)).y0(f).y1(g=>x(g.y)).curve(Y)).attr("fill",d.color).attr("opacity",e.fillAlpha),r.append("path").datum(d.kde).attr("d",Q().x(g=>h(g.x)).y(g=>x(g.y)).curve(Y)).attr("fill","none").attr("stroke",d.color).attr("stroke-width",2),e.showRug&&d.pts.forEach(g=>{r.append("line").attr("x1",h(g)).attr("x2",h(g)).attr("y1",f).attr("y2",f-6).attr("stroke",d.color).attr("opacity",.4)})});const u=r.append("g").attr("transform",`translate(${i+12},0)`);yt.forEach((d,g)=>{const v=u.append("g").attr("transform",`translate(0,${g*22})`);v.append("rect").attr("width",12).attr("height",3).attr("y",5).attr("fill",d.color).attr("rx",1),v.append("text").attr("x",16).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(d.grp)})}function Te(t,e){const s={top:20,right:80,bottom:60,left:70},{g:r,iW:i,iH:f}=T(t,540,380,s),b=nt.map(v=>v.gene),y=nt[0].values.map(v=>v.cond),p=R().domain(y).range([0,i]).padding(.04),h=R().domain(b).range([0,f]).padding(.04),c=nt.flatMap(v=>v.values.map(k=>k.val)),m=B(c.map(Math.abs)),l={rdbu:jt(xe).domain([m,0,-m]),viridis:Mt(ye).domain([-m,m]),plasma:Mt(ve).domain([-m,m])},n=l[e.colorScheme]||l.rdbu;r.append("g").attr("transform",`translate(0,${f})`).call(M(p)).call(v=>{v.select(".domain").attr("stroke","#424245"),v.selectAll(".tick line").remove(),v.selectAll("text").attr("fill","#a1a1a6").style("font-size","10px")}),r.append("g").call(E(h)).call(v=>{v.select(".domain").attr("stroke","#424245"),v.selectAll(".tick line").remove(),v.selectAll("text").attr("fill","#a1a1a6").style("font-size","10px").attr("font-style","italic")}),nt.forEach(v=>{v.values.forEach(k=>{r.append("rect").attr("x",p(k.cond)).attr("y",h(v.gene)).attr("width",p.bandwidth()).attr("height",h.bandwidth()).attr("fill",n(k.val)).attr("rx",3),e.showValues&&r.append("text").attr("x",p(k.cond)+p.bandwidth()/2).attr("y",h(v.gene)+h.bandwidth()/2+4).attr("text-anchor","middle").attr("fill",Math.abs(k.val)>m*.5?"#fff":"#444").style("font-size","9px").style("font-family","monospace").text(k.val.toFixed(1))})});const x=f,u=14,d=r.append("defs").append("linearGradient").attr("id","p7-hm-grad").attr("x1",0).attr("y1",1).attr("x2",0).attr("y2",0);[0,.25,.5,.75,1].forEach(v=>{d.append("stop").attr("offset",`${v*100}%`).attr("stop-color",n(-m+v*2*m))}),r.append("rect").attr("x",i+20).attr("y",0).attr("width",u).attr("height",x).attr("fill","url(#p7-hm-grad)").attr("rx",3);const g=_().domain([-m,m]).range([x,0]);r.append("g").attr("transform",`translate(${i+20+u},0)`).call(ie(g).ticks(4).tickFormat(Ct(".1f"))).call(v=>{v.select(".domain").remove(),v.selectAll(".tick line").attr("stroke","#424245"),v.selectAll("text").attr("fill","#a1a1a6").style("font-size","9px")})}function Fe(t,e){const s={top:25,right:110,bottom:55,left:55},{g:r,iW:i,iH:f}=T(t,540,380,s),b=["W0","W1","W2","W3","W4"],y=Pt().domain(b).range([0,i]).padding(.2),p=b.map((u,d)=>{const g={w:u};return V.forEach(v=>{g[v.grp]=v.pts[d].val}),g}),h=V.map(u=>u.grp),c=V.map(u=>u.color),m=(u,d)=>X().x((g,v)=>y(b[v])).y0(g=>d(g[0])).y1(g=>d(g[1])).curve(Wt),l=(u,d)=>Q().x((g,v)=>y(b[v])).y(g=>d(g[1])).curve(Wt);let n;if(e.arrangement==="proportional"){const u=pt().keys(h).offset(Ee)(p);n=_().domain([0,1]).range([f,0]),r.append("g").call(E(n).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(y)).call($),r.append("g").call(E(n).ticks(5).tickFormat(Ct(".0%"))).call($),A(r,"Week",!1,i,f),A(r,"Proportion",!0,i,f),u.forEach((d,g)=>{r.append("path").datum(d).attr("d",m(d,n)).attr("fill",c[g]).attr("opacity",e.fillAlpha),e.showLines&&r.append("path").datum(d).attr("d",l(d,n)).attr("fill","none").attr("stroke",c[g]).attr("stroke-width",1.5).attr("opacity",.8)})}else{const u=pt().keys(h)(p),d=B(u[u.length-1],g=>g[1]);n=_().domain([0,d*1.05]).range([f,0]),r.append("g").call(E(n).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(y)).call($),r.append("g").call(E(n).ticks(5)).call($),A(r,"Week",!1,i,f),A(r,"Response",!0,i,f),u.forEach((g,v)=>{r.append("path").datum(g).attr("d",m(g,n)).attr("fill",c[v]).attr("opacity",e.fillAlpha),e.showLines&&r.append("path").datum(g).attr("d",l(g,n)).attr("fill","none").attr("stroke",c[v]).attr("stroke-width",1.5).attr("opacity",.8)})}const x=r.append("g").attr("transform",`translate(${i+12},0)`);V.forEach((u,d)=>{const g=x.append("g").attr("transform",`translate(0,${d*22})`);g.append("rect").attr("width",12).attr("height",12).attr("fill",u.color).attr("opacity",e.fillAlpha).attr("rx",2),g.append("text").attr("x",16).attr("y",11).attr("fill","#a1a1a6").style("font-size","10px").text(u.grp)})}function Re(t,e){const s={top:25,right:30,bottom:65,left:55},{g:r,iW:i,iH:f}=T(t,540,380,s),b=e.errorType==="se"?"se":e.errorType==="sd"?"sd":"ci95",y=B(lt,l=>l.mean+l[b]),p=Math.max(0,D(lt,l=>l.mean-l[b])),h=R().domain(lt.map(l=>l.grp)).range([0,i]).padding(.5),c=_().domain([p*.85,y*1.15]).range([f,0]);r.append("g").call(E(c).ticks(5).tickSize(-i).tickFormat("")).call(W),r.append("g").attr("transform",`translate(0,${f})`).call(M(h)).call($).selectAll("text").attr("transform","rotate(-30)").attr("text-anchor","end").attr("dy","0.4em"),r.append("g").call(E(c).ticks(5)).call($),A(r,"Mean Value",!0,i,f),lt.forEach((l,n)=>{const x=h(l.grp)+h.bandwidth()/2,u=l[b],d=n<3?"#7EC8E3":"#F0B27A",g=e.capWidth*h.bandwidth();r.append("line").attr("x1",x).attr("x2",x).attr("y1",c(l.mean+u)).attr("y2",c(l.mean-u)).attr("stroke",d).attr("stroke-width",e.lineWidth),[-u,u].forEach(v=>{r.append("line").attr("x1",x-g).attr("x2",x+g).attr("y1",c(l.mean+v)).attr("y2",c(l.mean+v)).attr("stroke",d).attr("stroke-width",e.lineWidth)}),r.append("circle").attr("cx",x).attr("cy",c(l.mean)).attr("r",e.dotSize/2).attr("fill",d).attr("stroke",Et).attr("stroke-width",1.5)});const m={se:"Error bars: ±SE",sd:"Error bars: ±SD",ci95:"Error bars: 95% CI"};r.append("text").attr("x",i).attr("y",14).attr("text-anchor","end").attr("fill","#a1a1a6").style("font-size","10px").text(m[e.errorType])}function He(t,e){const a=e.horizontal?55:100,o=540,s=380,r={top:25,right:30,bottom:a,left:e.horizontal?80:55},{g:i,iW:f,iH:b}=T(t,o,s,r);let y=[...Be];e.sortBy==="asc"&&y.sort((h,c)=>h.val-c.val),e.sortBy==="desc"&&y.sort((h,c)=>c.val-h.val);const p=B(y,h=>Math.abs(h.val))*1.15;if(e.horizontal){const h=R().domain(y.map(l=>l.gene)).range([0,b]).padding(.4),c=_().domain([-p,p]).range([0,f]),m=c(0);i.append("g").call(E(h).ticks(0).tickSize(-f).tickFormat("")).call(W),i.append("g").call(M(c).ticks(5).tickSize(-b).tickFormat("")).call(W),i.append("g").call(E(h)).call(l=>{l.select(".domain").attr("stroke","#424245"),l.selectAll(".tick line").remove(),l.selectAll("text").attr("fill","#a1a1a6").style("font-size","10px").attr("font-style","italic")}),i.append("g").attr("transform",`translate(0,${b})`).call(M(c).ticks(5)).call($),i.append("line").attr("x1",m).attr("x2",m).attr("y1",0).attr("y2",b).attr("stroke","rgba(255,255,255,0.15)").attr("stroke-width",1),A(i,"log2 Fold Change",!1,f,b),y.forEach(l=>{const n=h(l.gene)+h.bandwidth()/2,x=l.val>0?"#F0B27A":"#7EC8E3";i.append("line").attr("x1",m).attr("x2",c(l.val)).attr("y1",n).attr("y2",n).attr("stroke",x).attr("stroke-width",e.lineWidth).attr("opacity",.7),i.append("circle").attr("cx",c(l.val)).attr("cy",n).attr("r",e.dotSize/2).attr("fill",x)})}else{const h=R().domain(y.map(l=>l.gene)).range([0,f]).padding(.4),c=_().domain([-p,p]).range([b,0]),m=c(0);i.append("g").call(E(c).ticks(5).tickSize(-f).tickFormat("")).call(W),i.append("g").attr("transform",`translate(0,${b})`).call(M(h)).call($).selectAll("text").attr("transform","rotate(-35)").attr("text-anchor","end").attr("dy","0.4em").attr("font-style","italic"),i.append("g").call(E(c).ticks(5)).call($),i.append("line").attr("x1",0).attr("x2",f).attr("y1",m).attr("y2",m).attr("stroke","rgba(255,255,255,0.15)").attr("stroke-width",1),A(i,"Gene",!1,f,b),A(i,"log2 FC",!0,f,b),y.forEach(l=>{const n=h(l.gene)+h.bandwidth()/2,x=l.val>0?"#F0B27A":"#7EC8E3";i.append("line").attr("x1",n).attr("x2",n).attr("y1",m).attr("y2",c(l.val)).attr("stroke",x).attr("stroke-width",e.lineWidth).attr("opacity",.7),i.append("circle").attr("cx",n).attr("cy",c(l.val)).attr("r",e.dotSize/2).attr("fill",x)})}}function Pe(t,e){const s={top:30,right:20,bottom:50,left:90},{g:r,iW:i,iH:f}=T(t,540,380,s),b=st.flatMap(v=>v.pts),y=D(b)-5,p=B(b)+5,h=_().domain([y,p]).range([0,i]),c=mt(e.bw*5),m=ht(y,p,(p-y)/100),l=st.length,n=f/(l+.5),x=st.map(v=>bt(v.pts,c,m)),u=B(x,v=>B(v,k=>k.y)),d=n*(1+e.overlap),g=["#7EC8E3","#F0B27A","#95D5B2","#B8B8E8"];r.append("g").attr("transform",`translate(0,${f})`).call(M(h).ticks(6)).call($),A(r,"Value",!1,i,f),st.forEach((v,k)=>{const P=x[k],j=f-k*n,q=g[k%g.length],C=_().domain([0,u]).range([0,-d]),I=r.append("g").attr("transform",`translate(0,${j})`);I.append("path").datum(P).attr("d",X().x(F=>h(F.x)).y0(0).y1(F=>C(F.y)).curve(Y)).attr("fill",q).attr("opacity",e.fillAlpha),I.append("line").attr("x1",0).attr("x2",i).attr("y1",0).attr("y2",0).attr("stroke","rgba(255,255,255,0.1)").attr("stroke-width",1),I.append("path").datum(P).attr("d",Q().x(F=>h(F.x)).y(F=>C(F.y)).curve(Y)).attr("fill","none").attr("stroke",q).attr("stroke-width",2),I.append("text").attr("x",-8).attr("y",0).attr("text-anchor","end").attr("fill","#a1a1a6").style("font-size","10px").attr("dominant-baseline","middle").text(v.tp)})}const Ie={scatter:_e,bar:Le,line:Me,boxplot:We,violin:Se,histogram:De,density:ze,heatmap:Te,area:Fe,errorbar:Re,lollipop:He,ridgeline:Pe},qe={circle:"16",triangle:"17",square:"15",cross:"3"},Ve={solid:'"solid"',dashed:'"dashed"',dotted:'"dotted"'};function je(t){const e=t.jitter!=="none"?`    position = position_jitter(
...
ggplot(data, aes(
  ...
  <input class="p7-slider" type="range" data-param="${t}"
    min="${e}" max="${a}" step="${o}" value="${s}">`}function ea(t){return`
    ...
    <div class="p7-ctrl-group">${L("bw",.5,5,.1,t.bw)}</div>
    <button class="p7-reset-btn" data-reset="true">↺ 重置参数</button>`}const ha={scatter:ea,bar:aa,line:ra,boxplot:oa,violin:ia,histogram:na,density:la,heatmap:sa,area:ca,errorbar:da,lollipop:pa,ridgeline:fa};function Da(){const t=`<svg class="p7-chevron" width="16" height="16" viewBox="0 0 24 24"
