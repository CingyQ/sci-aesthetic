// p04-poster-and-ga.js — 学术海报与 GA
// Hero → S1 海报 vs GA 对比 → S2 海报布局模板 → S3 海报设计要点
//      → S4 GA 设计流程 → S5 GA 案例评分 → S6 尺寸速查 → Footer
...
let _layoutScrollHandler = null;
let _activeLayoutIndex = 0;
let _activeFlowStep = 0;
...
const LAYOUTS = [
  {
    name: '三栏经典',
    desc: '最传统，适合内容丰富的研究。左背景+中方法结果+右结论。',
    render: () => `
  ...
  {
    name: '信息流',
    desc: '单列蛇形流，适合流程类研究，叙事性强，引导阅读顺序。',
    render: () => `
  ...
  <div class="pga-section-inner">
    ...
    <div class="pga-layout-wrap" id="pga-layout-wrap">
      ...
      <div class="pga-layout-list" id="pga-layout-list">
        ${LAYOUTS.map((l, i) => `
        <div class="pga-layout-card${i === 0 ? ' pga-layout-active' : ''}" data-layout-idx="${i}">
          <div class="pga-layout-card-name">${i + 1}. ${l.name}</div>
          <div class="pga-layout-card-desc">${l.desc}</div>
        </div>`).join('')}
      </div>
      <!-- 右侧预览 -->
...
export function init() {
  ...
  // ── S1 滚动动画 ──
  fadeIn('.pga-compare-wrap .section-header', { y: 40, duration: 0.8 });
  fadeIn('.pga-compare-card', { stagger: 0.1, y: 30, duration: 0.7 });

  // ── S2 布局列表点击 + JS Sticky 预览 ──
  _initLayoutSection();

  // ── S3 滚动动画 ──
  fadeIn('#pga-poster-tips .section-header', { y: 40, duration: 0.8 });
  fadeIn('.pga-tip-card', { stagger: 0.1, y: 30, duration: 0.6 });

  // ── S4 GA 流程步骤 ──
  _initFlowSteps();
  fadeIn('#pga-ga-flow .section-header', { y: 40, duration: 0.8 });

  // ── S5 案例评分 ──
  fadeIn('#pga-ga-rating .section-header', { y: 40, duration: 0.8 });
  fadeIn('.pga-rating-card', { stagger: 0.12, y: 30, duration: 0.7 });

  // ── S6 TabSwitcher ──
  _initSizesTab();

  // ── Footer ──
  fadeIn('.page-footer-num', { y: 20, duration: 0.7 });
  fadeIn('.page-footer-quote', { y: 30, duration: 0.8 });
  fadeIn('.page-footer-desc', { y: 20, duration: 0.7 });
  fadeIn('.pga-footer-nav', { y: 20, duration: 0.7 });

  addEvt(document.getElementById('pga-btn-prev'), 'click', () => navigateTo('m4-p3'));
  addEvt(document.getElementById('pga-btn-next'), 'click', () => navigateTo('home'));
}

// ── S2 布局模板逻辑 ──
function _initLayoutSection() {
  const cards = document.querySelectorAll('.pga-layout-card');
  const previewEl = document.getElementById('pga-layout-preview');
  if (!previewEl) return;

  fadeIn('#pga-poster-layout .section-header', { y: 40, duration: 0.8 });
  fadeIn('.pga-layout-card', { stagger: 0.1, y: 30, duration: 0.6 });

  cards.forEach(card => {
    addEvt(card, 'click', () => {
      const idx = parseInt(card.dataset.layoutIdx, 10);
      if (idx === _activeLayoutIndex) return;
      _activeLayoutIndex = idx;

      // 更新激活状态
      cards.forEach(c => c.classList.remove('pga-layout-active'));
      card.classList.add('pga-layout-active');

      // GSAP 淡入预览
      gsap.to(previewEl, {
        opacity: 0, duration: 0.18, ease: 'power2.in',
        onComplete: () => {
          previewEl.innerHTML = LAYOUTS[idx].render();
  ...
  // JS sticky 预览（桌面端）
  const wrapEl = document.getElementById('pga-layout-wrap');
  const previewColEl = previewEl ? previewEl.parentElement : null;
...
// ── S6 尺寸 TabSwitcher ──
function _initSizesTab() {
  const container = document.getElementById('pga-sizes-tabs');
  if (!container) return;

  const switcher = createTabSwitcher(container, {
    tabs: [
      { id: 'poster', label: '海报尺寸' },
      { id: 'ga',     label: 'GA 期刊要求' }
    ],
    activeId: 'poster',
    variant: 'default',
    onChange: (id) => {
      document.querySelectorAll('.pga-sizes-tab-content').forEach(el => {
        el.classList.remove('pga-tab-active');
      });
      const target = document.getElementById(`pga-sizes-content-${id}`);
      if (target) {
        gsap.fromTo(target,
          { opacity: 0, y: 10 },
          { opacity: 1, y: 0, duration: 0.4, ease: 'power2.out',
            onStart: () => { target.classList.add('pga-tab-active'); }
          }
        );
      }
    }
  });
  _tabSwitchers.push(switcher);

  fadeIn('#pga-sizes .section-header', { y: 40, duration: 0.8 });
  fadeIn('#pga-sizes-tabs', { y: 30, duration: 0.6 });
}
