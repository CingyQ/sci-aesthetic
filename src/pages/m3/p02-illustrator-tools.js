...
const TOOLS = [
  {
    id: 'select', name: '选择工具', shortcut: 'V', category: 'select',
    desc: '选中、移动、缩放和旋转完整对象。按住 Shift 多选，拖拽角点等比缩放。',
    use: '选中并整体移动图表元素、调整整体尺寸',
  ...
  {
    id: 'direct', name: '直接选择', shortcut: 'A', category: 'select',
    desc: '单独编辑锚点和路径段。点击选中锚点（实心方块），拖拽改变路径形状。',
    use: '微调流程图连接线的弯曲程度、箭头形状',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  ...
  {
    id: 'pen', name: '钢笔工具', shortcut: 'P', category: 'draw',
    desc: '点击创建角点，拖拽创建贝塞尔曲线控制手柄。Shift 约束 45° 角，Alt 断开手柄。',
    use: '绘制自定义箭头、弯曲连接线、有机轮廓形状',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  ...
  {
    id: 'type', name: '文字工具', shortcut: 'T', category: 'text',
    desc: '点击添加点文字，拖拽创建区域文字框。双击编辑已有文字。支持 OpenType 特性。',
    use: '添加图表标题、坐标轴标签、图注标注文字',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  ...
  },
  {
    ...
    desc: '布尔运算：联集/交集/差集/分割。将多个基本形状组合成复杂自定义形状。',
    use: '制作自定义流程图节点形状、挖空文字效果、复合箭头',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  <g class="pf-before">
  ...
  {
    id: 'gradient', name: '渐变工具', shortcut: 'G', category: 'color',
    desc: '在选中对象上拖拽定义渐变方向和范围。支持线性渐变和径向渐变，可在画布直接添加/移动色标。',
    use: '为背景添加渐变过渡、突出重点区域、制作深度感',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  <defs>
  ...
  {
    id: 'eyedropper', name: '吸管工具', shortcut: 'I', category: 'color',
    desc: '从画面任意位置采样颜色（包括其他应用程序）。Option+点击应用颜色到选中对象。',
    use: '从参考图片/期刊截图中采样配色，统一论文所有图表色调',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  ...
  {
    id: 'align', name: '对齐工具', shortcut: 'Shift+F7', category: 'layout',
    desc: '精确对齐和分布多个对象。支持对齐到画板、关键对象或选区。消除手动排列的不精确性。',
    use: '确保多面板图表间距精确均等、标注文字与图表对齐',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  ...
  {
    id: 'group', name: '编组', shortcut: 'Ctrl+G', category: 'layout',
    desc: '将多个对象组合为一个整体，方便整体移动和缩放。双击进入组内编辑单个元素。',
    use: '将图表各元素（轴/点/标注）打包，一键整体移动到 Figure 指定位置',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  ...
  {
    id: 'artboard', name: '画板工具', shortcut: 'Shift+O', category: 'layout',
    desc: '创建和管理多个画板，每个画板可设置独立尺寸。支持按照期刊规格精确设置（如 86mm 单栏宽）。',
    use: '管理 a/b/c/d 多面板 Figure，按期刊规格设置导出尺寸',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
  ...
  {
    id: 'zoom', name: '缩放工具', shortcut: 'Z', category: 'view',
  ...
  {
    id: 'scissors', name: '剪刀工具', shortcut: 'C', category: 'draw',
    desc: '在路径上的锚点处剪断路径，将一条路径分成两条独立路径。',
    use: '将自动生成的封闭路径剪开、分离图表区域轮廓用于独立着色',
    svgDemo: `<svg viewBox="0 0 160 120" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:100%">
...
const WORKFLOWS = [
  {
    ...
    title: '工作流 A：绘制弯曲箭头',
    steps: [
      { tool: '钢笔 (P)', action: '绘制弯曲路径', note: '点击+拖拽创建贝塞尔手柄' },
      { tool: '描边→轮廓化', action: 'Shift+Ctrl+O', note: '将描边转为可填色路径' },
      { tool: 'Pathfinder', action: '联集合并', note: '箭头体+箭头头合并' },
      { tool: '直接选择 (A)', action: '微调锚点', note: '精细调整箭头弧度' },
      { tool: '吸管 (I)', action: '采样配色', note: '匹配图表主色调' },
    ],
    output: '可复用矢量箭头符号（存为图形样式）'
  },
  {
    id: 'unify',
    title: '工作流 B：统一多图表配色',
    steps: [
      { tool: '选择 (V)', action: '框选所有图表', note: '或 Ctrl+A 全选' },
      { tool: '吸管 (I)', action: '从参考图采色', note: '获取期刊官方配色' },
      { tool: '重新着色', action: 'Shift+Ctrl+F', note: '将相近颜色合并统一' },
      { tool: '编组 (Ctrl+G)', action: '按图表分组', note: '方便后续定位管理' },
      { tool: '画板 (Shift+O)', action: '各图表单独导出', note: '按期刊尺寸导出 PDF' },
    ],
    output: '配色统一的多图表集（可直接投稿）'
  },
  {
    id: 'multipanel',
    title: '工作流 C：多面板 Figure',
    steps: [
      { tool: '画板 (Shift+O)', action: '创建 4 个画板', note: '按期刊单双栏尺寸设置' },
      { tool: '导入子图', action: '置入各面板图', note: 'File→Place 置入 PDF/SVG' },
      { tool: '对齐 (Shift+F7)', action: '精确对齐间距', note: '分布间距设为 2mm' },
      { tool: '文字 (T)', action: '添加 a/b/c/d 标注', note: '8pt 粗体，左上角偏移 2mm' },
      { tool: 'File→Export', action: '导出出版格式', note: '存储为 PDF + 导出 TIFF 300dpi' },
    ],
    output: '出版级多面板 Figure（符合期刊提交规范）'
  }
];

// 快捷键数据
const SHORTCUTS = [
  { key: 'V',             cat: 'select', name: '选择工具',    scenario: '选中/移动整个图表元素' },
  { key: 'A',             cat: 'select', name: '直接选择',    scenario: '编辑单个锚点和路径' },
  { key: 'Ctrl+G',        cat: 'select', name: '编组',        scenario: '将图表元素打包整体操作' },
  { key: 'Ctrl+Shift+G',  cat: 'select', name: '解组',        scenario: '进入组内编辑单个元素' },
  { key: 'P',             cat: 'draw',   name: '钢笔工具',    scenario: '绘制弯曲连接线和箭头' },
  { key: 'C',             cat: 'draw',   name: '剪刀工具',    scenario: '剪断路径分离图表区域' },
  { key: 'Shift+Ctrl+O',  cat: 'draw',   name: '轮廓化描边',  scenario: '将描边转为可填色路径（制作箭头必备）' },
  { key: 'T',             cat: 'text',   name: '文字工具',    scenario: '添加标题/标注/坐标轴标签' },
  { key: 'Shift+Ctrl+F9', cat: 'shape',  name: 'Pathfinder',  scenario: '打开布尔运算面板（联集/差集）' },
  { key: 'Shift+F7',      cat: 'view',   name: '对齐面板',    scenario: '精确对齐和均等分布多个对象' },
  { key: 'G',             cat: 'view',   name: '渐变工具',    scenario: '在对象上直接定义渐变方向' },
  { key: 'Cmd+0',         cat: 'view',   name: '适合窗口',    scenario: '查看整体布局，导出前检查' },
];

const SHORTCUT_CATS = [
  { id: 'all',    label: '全部' },
  { id: 'select', label: '选择与编组' },
  { id: 'draw',   label: '绘制' },
  { id: 'text',   label: '文字' },
  { id: 'shape',  label: '形状' },
  { id: 'view',   label: '视图与对齐' },
...
export function init() {
  ...
  heroTl.fromTo('#p02-hero .hero-eyebrow',   { opacity:0, y:20 }, { opacity:1,   y:0, duration:0.6, ease:'power3.out' }, 0);
  heroTl.fromTo('#p02-hero .page-hero-title', { y:30, opacity:0 }, { opacity:1,   y:0, duration:0.8, ease:'power3.out' }, 0.15);
  heroTl.fromTo('#p02-hero .page-hero-sub',   { y:20, opacity:0 }, { opacity:0.5, y:0, duration:0.8, ease:'power3.out' }, 0.3);
  heroTl.fromTo('.p02-hero-tagline',          { y:20, opacity:0 }, { opacity:1,   y:0, duration:0.8, ease:'power3.out' }, 0.45);
  heroTl.fromTo('#p02-quicknav',              { y:20, opacity:0 }, { opacity:1,   y:0, duration:0.8, ease:'power3.out' }, 0.6);
  heroTl.fromTo('.p02-scroll-hint',           { opacity:0, y:15 }, { opacity:1,   y:0, duration:0.6, ease:'power3.out' }, 0.75);

  // Quicknav 滚动
  document.querySelectorAll('#p02-quicknav .hero-quicknav__item').forEach(btn => {
    const fn = () => {
      document.querySelector(btn.dataset.target)?.scrollIntoView({ behavior:'smooth', block:'start' });
    };
    btn.addEventListener('click', fn);
    _eventHandlers.push({ el: btn, type: 'click', fn });
  });
  ...
  const catTabsEl   = document.getElementById('p02-cat-tabs');
  const toolListEl  = document.getElementById('p02-tool-list');
  ...
  function renderCatTabs() {
    ...
    CATEGORIES.forEach(cat => {
      ...
      btn.className = `p02-cat-tab${cat.id === activeCat ? ' active' : ''}`;
  ...
  function renderToolList() {
    ...
    filtered.forEach(tool => {
      const shortDisplay = tool.shortcut.length > 8 ? tool.shortcut.split('+').pop() : tool.shortcut;
      const item = document.createElement('div');
      item.className = `p02-tool-item${tool.id === activeTool.id ? ' active' : ''}`;
      item.dataset.toolId = tool.id;
      item.innerHTML = `
        <span class="p02-tool-shortcut">${shortDisplay}</span>
        <span class="p02-tool-name">${tool.name}</span>
      `;
      toolListEl.appendChild(item);
    });
  }

  function renderDemoPanel(tool) {
    const titleEl    = document.getElementById('p02-demo-title');
    const shortcutEl = document.getElementById('p02-demo-shortcut');
    const svgEl      = document.getElementById('p02-demo-svg');
    const descEl     = document.getElementById('p02-demo-desc');
    const useEl      = document.getElementById('p02-demo-use-text');
    if (!titleEl || !svgEl) return;
    titleEl.textContent    = tool.name;
    shortcutEl.textContent = tool.shortcut;
    svgEl.innerHTML        = tool.svgDemo;
    descEl.textContent     = tool.desc;
    useEl.textContent      = tool.use;
  }

  renderCatTabs();
  renderToolList();
  renderDemoPanel(activeTool);

  // 分类 Tab 点击（事件委托）
  const catTabClickFn = (e) => {
    const btn = e.target.closest('.p02-cat-tab');
    if (!btn) return;
    activeCat = btn.dataset.cat;
    renderCatTabs();
    const filtered = activeCat === 'all' ? TOOLS : TOOLS.filter(t => t.category === activeCat);
    if (filtered.length && !filtered.find(t => t.id === activeTool.id)) {
      activeTool = filtered[0];
      renderDemoPanel(activeTool);
    }
    renderToolList();
  };
  catTabsEl?.addEventListener('click', catTabClickFn);
  _eventHandlers.push({ el: catTabsEl, type: 'click', fn: catTabClickFn });

  // 工具列表点击（事件委托）
  const toolListClickFn = (e) => {
    const item = e.target.closest('.p02-tool-item');
    if (!item) return;
    const tool = TOOLS.find(t => t.id === item.dataset.toolId);
    if (!tool) return;
    activeTool = tool;
    renderDemoPanel(tool);
    toolListEl?.querySelectorAll('.p02-tool-item').forEach(el => {
      el.classList.toggle('active', el.dataset.toolId === tool.id);
    });
  };
  toolListEl?.addEventListener('click', toolListClickFn);
  _eventHandlers.push({ el: toolListEl, type: 'click', fn: toolListClickFn });

  // 重播动画
  const replayBtn = document.getElementById('p02-replay-btn');
  if (replayBtn) {
    const replayFn = () => {
      const svgEl = document.getElementById('p02-demo-svg');
      if (!svgEl) return;
      ...
      svgEl.innerHTML = '';
      requestAnimationFrame(() => { svgEl.innerHTML = html; });
    };
    replayBtn.addEventListener('click', replayFn);
    _eventHandlers.push({ el: replayBtn, type: 'click', fn: replayFn });
  }

  // S1 入场
  const s1El = document.getElementById('p02-s1');
  if (s1El) {
    const s1io = new IntersectionObserver(([e]) => {
      if (!e.isIntersecting) return;
      fadeIn(document.querySelectorAll('#p02-s1 .reading-wrapper'), { stagger:0.1, y:30 });
      gsap.fromTo('.p02-browser', { opacity:0, y:40 }, { opacity:1, y:0, duration:0.8, ease:'power3.out', delay:0.2 });
      s1io.disconnect();
    }, { threshold: 0.1 });
    s1io.observe(s1El);
    _observers.push(s1io);
  }

  // ── 3. S2 工作流演示 ──
  const wfTabsEl   = document.getElementById('p02-wf-tabs');
  const wfPanelsEl = document.getElementById('p02-wf-panels');

  function buildWorkflowPanel(wf) {
    let stepsHtml = '<div class="p02-wf-steps">';
    wf.steps.forEach((step, i) => {
      stepsHtml += `
        <div class="p02-wf-step" style="opacity:0;" data-wf-step>
          <div class="p02-wf-step-card">
            <div class="p02-wf-step-tool">${step.tool}</div>
            <div class="p02-wf-step-action">${step.action}</div>
            <div class="p02-wf-step-note">${step.note}</div>
          </div>
        </div>`;
      if (i < wf.steps.length - 1) {
        stepsHtml += `<div class="p02-wf-arrow" aria-hidden="true">→</div>`;
      }
    });
    stepsHtml += '</div>';
    return `${stepsHtml}
  ...
  function animateWfSteps(panelIdx) {
    const panel = document.getElementById(`p02-wf-panel-${panelIdx}`);
    if (!panel) return;
    const steps = panel.querySelectorAll('[data-wf-step]');
    gsap.fromTo(steps,
      { opacity:0, y:30 },
  ...
  if (wfTabsEl && wfPanelsEl) {
    WORKFLOWS.forEach((wf, i) => {
      const tab = document.createElement('button');
      tab.className = `p02-wf-tab${i === 0 ? ' active' : ''}`;
      tab.textContent = wf.title;
      tab.dataset.wfIdx = i;
      wfTabsEl.appendChild(tab);

      const panel = document.createElement('div');
      panel.className = `p02-wf-panel${i === 0 ? ' active' : ''}`;
      panel.id = `p02-wf-panel-${i}`;
      panel.innerHTML = buildWorkflowPanel(wf);
      wfPanelsEl.appendChild(panel);
    });

    const s2El = document.getElementById('p02-s2');
    if (s2El) {
      const s2io = new IntersectionObserver(([e]) => {
        if (!e.isIntersecting) return;
        fadeIn(document.querySelectorAll('#p02-s2 .reading-wrapper'), { stagger:0.1, y:30 });
        animateWfSteps(0);
        s2io.disconnect();
      }, { threshold: 0.15 });
      s2io.observe(s2El);
      _observers.push(s2io);
    }

    const wfTabClickFn = (e) => {
      const tab = e.target.closest('.p02-wf-tab');
      if (!tab) return;
      const idx = parseInt(tab.dataset.wfIdx, 10);
      wfTabsEl.querySelectorAll('.p02-wf-tab').forEach(t => t.classList.remove('active'));
      wfPanelsEl.querySelectorAll('.p02-wf-panel').forEach(p => p.classList.remove('active'));
      tab.classList.add('active');
      document.getElementById(`p02-wf-panel-${idx}`)?.classList.add('active');
      animateWfSteps(idx);
    };
    wfTabsEl.addEventListener('click', wfTabClickFn);
    _eventHandlers.push({ el: wfTabsEl, type: 'click', fn: wfTabClickFn });
  }

  // ── 4. S3 快捷键速查 ──
  const scTabsEl  = document.getElementById('p02-sc-tabs');
  const scTbodyEl = document.getElementById('p02-sc-tbody');
  let activeScat  = 'all';

  function renderKbdKey(keyStr) {
    const parts = keyStr.split('+');
    return parts.map(k => `<kbd class="p02-kbd">${k.trim()}</kbd>`).join('<span style="color:var(--text-on-light-3);font-size:0.7rem;margin:0 1px;">+</span>');
  }

  function renderShortcuts() {
    if (!scTbodyEl) return;
    const filtered = activeScat === 'all' ? SHORTCUTS : SHORTCUTS.filter(s => s.cat === activeScat);
    scTbodyEl.innerHTML = filtered.map(s => `
      <tr>
        <td><span class="p02-sc-key">${renderKbdKey(s.key)}</span></td>
        <td><span class="p02-sc-name">${s.name}</span></td>
        <td><span class="p02-sc-scenario">${s.scenario}</span></td>
      </tr>
    `).join('');
  }
  ...
  function renderScTabs() {
    if (!scTabsEl) return;
    scTabsEl.innerHTML = '';
    SHORTCUT_CATS.forEach(cat => {
      const btn = document.createElement('button');
      btn.className = `p02-sc-tab${cat.id === activeScat ? ' active' : ''}`;
      btn.textContent = cat.label;
      btn.dataset.cat = cat.id;
  ...
  const scTabClickFn = (e) => {
    const btn = e.target.closest('.p02-sc-tab');
    if (!btn) return;
    activeScat = btn.dataset.cat;
    renderScTabs();
    renderShortcuts();
  };
  scTabsEl?.addEventListener('click', scTabClickFn);
  _eventHandlers.push({ el: scTabsEl, type: 'click', fn: scTabClickFn });

  const s3El = document.getElementById('p02-s3');
  if (s3El) {
    const s3io = new IntersectionObserver(([e]) => {
      if (!e.isIntersecting) return;
      fadeIn(document.querySelectorAll('#p02-s3 .reading-wrapper'), { stagger:0.1, y:30 });
      fadeIn(document.querySelectorAll('#p02-s3 .p02-sc-tabs, #p02-s3 .p02-sc-table'), { stagger:0.15, y:30 });
      s3io.disconnect();
    }, { threshold: 0.1 });
    s3io.observe(s3El);
    _observers.push(s3io);
  }
  ...
  if (nextBtn) {
    ...
    _eventHandlers.push({ el: nextBtn, type: 'click', fn });
  }
}
