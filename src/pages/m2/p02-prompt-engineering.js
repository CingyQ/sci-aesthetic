...
[Format] 2K resolution, 300 DPI, white background, academic publication standard.`,
  ...
  const compareGrid = document.getElementById('p02-compare-grid');
  if (compareGrid) {
    COMPARES.forEach(c => {
      const card = document.createElement('div');
      card.className = 'p02-compare-card';
  ...
  fadeIn(document.querySelectorAll('#p02-s2 .reading-wrapper, #p02-s2 .p02-compare-card'), { stagger: 0.1, y: 40 });
...
Footnote: 'Source: Millennium Ecosystem Assessment (MA 2005)'.`
  ...
  const iterTrack  = document.getElementById('p02-iter-track');
  const iterDetail = document.getElementById('p02-iter-detail');

  function renderIterDetail(idx) {
    const it = ITERATIONS[idx];
    if (!iterDetail) return;
      ...
      <div class="p02-iter-detail-grid">
        ...
        <div>
          ...
          <div style="margin-top:var(--space-md);background:var(--bg-dark);border-radius:var(--radius-sm);padding:var(--space-md);border:1px solid var(--border-dark);">
            ...
            <pre style="font-family:var(--font-code);font-size:0.78rem;color:var(--text-on-dark-2);white-space:pre-wrap;word-wrap:break-word;line-height:1.7;">${escapeHtml(it.prompt)}</pre>
          </div>
        </div>
      </div>`;
  }
  ...
  if (iterTrack) {
    ITERATIONS.forEach((it, i) => {
      ...
      item.className = `p02-iter-item${i === 0 ? ' active' : ''}`;
      item.dataset.idx = i;
      item.innerHTML = `
      ...
      iterTrack.appendChild(item);
    });
    renderIterDetail(0);
    iterTrack.addEventListener('click', e => {
      const item = e.target.closest('.p02-iter-item');
      if (!item) return;
      const idx = parseInt(item.dataset.idx);
      iterTrack.querySelectorAll('.p02-iter-item').forEach((el, i) => el.classList.toggle('active', i === idx));
      renderIterDetail(idx);
    });
  }
  fadeIn(document.querySelectorAll('#p02-s3 .reading-wrapper, #p02-iter-track'), { stagger: 0.1, y: 30 });

  // ── S4: Prompt 模板库 ──
  const TEMPLATES = [
    {
      icon: '🔵', label: '概念示意图', desc: '机制、过程、关系可视化',
      structure: `为《[期刊名]》制作[研究主题]概念示意图
[C] [研究背景/生态系统/实验条件]
[D] [核心科学过程，3-5个关键步骤]
[T] [视角：鸟瞰/剖面/3D] + [配色方案] + [标注字体/字号]
[F] 分辨率[DPI]，[色彩模式]，比例[W:H]，格式[PNG/SVG]`,
      example: `为《Environmental Science & Technology》制作土壤重金属修复概念示意图
[C] EDTA辅助植物修复，中国华北平原农业土壤
[D] EDTA施用→土壤重金属活化→植物根系吸收→地上部积累→收割移除
[T] 剖面视角，绿色植株+棕色土壤+蓝色箭头，Helvetica 10pt标注
[F] 300 DPI，RGB，1:1.2，PNG+SVG`
    },
    {
      icon: '🔬', label: '实验流程图', desc: '方法步骤、样品处理流程',
      structure: `实验方法流程图，[背景颜色]，[风格]
[C] [实验目的/方法体系]
[D] 步骤1: [样品来源] → 步骤2: [处理方法] → 步骤3: [分析测试] → 步骤4: [数据处理]
[T] 从左到右线性流程，每步配[图标类型]，[配色方案]
[F] 宽:高 = [比例]，分辨率 300 DPI`,
...
[F] 3:1宽高比，300 DPI，PNG`
    },
    {
      icon: '📋', label: 'Graphical Abstract', desc: '论文视觉摘要，信息密集型',
      structure: `期刊Graphical Abstract，白色背景，信息密集
[C] [论文主题一句话]，发表于[目标期刊]
[D] 左: [问题/背景] → 中: [核心方法/发现] → 右: [意义/结论]
关键数据：[最重要的1-3个量化结果]
[T] 左-中-右三栏，关键数字放大，箭头引导阅读
[F] 宽:高=1.8:1，RGB，300 DPI，白色背景`,
      example: `期刊Graphical Abstract，白色背景
...
[F] 1.8:1，RGB，300 DPI`
    ...
    {
      icon: '🗺', label: '综述框架图', desc: '概念关系、研究体系框架',
      structure: `综述框架概念图，学术风格，[配色]
[C] [综述主题]，涵盖[几个]核心方向
[D] [主题] → [子主题1/子主题2/子主题3] → [各子主题核心内容]
[T] [层级关系/同心圆/模块化布局]，关联线展示关系
[F] [方形/竖版]，300 DPI，RGB`,
      example: `综述框架概念图，深蓝白配色，学术风格
[C] 微塑料在土壤-植物系统行为与效应综述，4个核心方向
[D] 微塑料土壤效应 → ①土壤物理结构 ②微生物群落 ③植物吸收毒性 ④食物链传递
各方向标注代表研究方法和关键发现
[T] 中心主题+四象限辐射，关联箭头，关键词加框
[F] 1:1正方形，300 DPI，RGB`
    },
    {
      icon: '📊', label: '数据可视化辅助', desc: '图表底图、配套示意图',
...
[F] 600×500px，300 DPI，PNG透明背景`
    },
  ];

  const tplList    = document.getElementById('p02-tpl-list');
  const tplPreview = document.getElementById('p02-tpl-preview');

  function renderTplPreview(idx) {
    const t = TEMPLATES[idx];
    if (!tplPreview) return;
    // 高亮模板中的占位符
    const highlighted = escapeHtml(t.structure).replace(/\[([^\]]+)\]/g,
      '<span class="p02-tpl-placeholder">[$1]</span>');
    tplPreview.innerHTML = `
      <div class="p02-tpl-preview">
        <h3>${t.icon} ${t.label}</h3>
        <div class="p02-tpl-structure">${highlighted}</div>
        <div class="p02-tpl-example-label">示例（环境科学）</div>
        <div class="p02-tpl-example">${escapeHtml(t.example)}</div>
        <button class="p02-copy-btn" id="p02-copy-tpl">📋 复制模板</button>
      </div>`;
    const copyBtn = document.getElementById('p02-copy-tpl');
    if (copyBtn) {
      copyBtn.addEventListener('click', () => {
        navigator.clipboard?.writeText(t.structure).then(() => {
          copyBtn.textContent = '✅ 已复制！';
          copyBtn.classList.add('copied');
          setTimeout(() => { copyBtn.textContent = '📋 复制模板'; copyBtn.classList.remove('copied'); }, 2000);
        }).catch(() => {
          // 降级方案
          const ta = document.createElement('textarea');
          ta.value = t.structure;
          document.body.appendChild(ta);
          ta.select();
          document.execCommand('copy');
          document.body.removeChild(ta);
          copyBtn.textContent = '✅ 已复制！';
          setTimeout(() => { copyBtn.textContent = '📋 复制模板'; }, 2000);
        });
      });
    }
  }
  ...
  if (tplList) {
    TEMPLATES.forEach((t, i) => {
      const item = document.createElement('div');
      item.className = `p02-tpl-item${i === 0 ? ' active' : ''}`;
      item.dataset.idx = i;
      item.innerHTML = `<div class="p02-tpl-icon">${t.icon}</div><div class="p02-tpl-label">${t.label}</div><div class="p02-tpl-desc">${t.desc}</div>`;
      tplList.appendChild(item);
    });
    renderTplPreview(0);
    tplList.addEventListener('click', e => {
      const item = e.target.closest('.p02-tpl-item');
      if (!item) return;
      const idx = parseInt(item.dataset.idx);
      tplList.querySelectorAll('.p02-tpl-item').forEach((el, i) => el.classList.toggle('active', i === idx));
      renderTplPreview(idx);
    });
  }
  fadeIn(document.querySelectorAll('#p02-s4 .reading-wrapper, #p02-tpl-list'), { stagger: 0.1, y: 30 });

  // ── Footer ──
  document.getElementById('p02-prev-btn')?.addEventListener('click', () => navigateTo('m2-p1'));
  document.getElementById('p02-next-btn')?.addEventListener('click', () => navigateTo('m2-p3'));
}
