...
export function init() {
  ...
  // ── 2. S1: 双时间线 ──
  const TRAD = [
    { n:'01', title:'手绘草稿', time:'~30 min', skill:'纸笔构思' },
    { n:'02', title:'软件建图', time:'~2 hr',   skill:'Illustrator/PPT' },
    { n:'03', title:'配色调试', time:'~45 min', skill:'色彩审美' },
    { n:'04', title:'标注排版', time:'~1 hr',   skill:'版式设计' },
    { n:'05', title:'格式导出', time:'~20 min', skill:'技术参数' },
    { n:'06', title:'反复修改', time:'循环多次', skill:'耐心' },
  ];
  const AI_STEPS = [
    { n:'01', title:'CDTF 描述撰写', time:'~15 min', skill:'科学表达' },
    { n:'02', title:'AI 生成初稿',   time:'~2 min',  skill:'Prompt 迭代' },
    { n:'03', title:'迭代精修',       time:'~30 min', skill:'审美判断' },
    { n:'04', title:'矢量化后处理',   time:'~45 min', skill:'Illustrator' },
  ];
  ...
  function buildTimeline(containerId, steps) {
    ...
    steps.forEach((s, i) => {
      ...
      step.className = 'p01-tl-step';
      step.dataset.tlIdx = i;
      step.innerHTML = `<div class="p01-tl-step-num">${s.n}</div>
        <div style="flex:1;"><div class="p01-tl-step-title">${s.title}</div>
        <div class="p01-tl-badges"><span class="p01-badge p01-badge-time">${s.time}</span><span class="p01-badge p01-badge-skill">${s.skill}</span></div>
        </div>`;
      col.appendChild(step);
      if (i < steps.length - 1) {
        const conn = document.createElement('div');
        conn.className = 'p01-tl-connector';
        col.appendChild(conn);
      }
    });
  }

  buildTimeline('p01-tl-trad', TRAD);
  buildTimeline('p01-tl-ai', AI_STEPS);

  // Activate steps via IntersectionObserver
  document.querySelectorAll('#p01-s1 .p01-tl-step').forEach(step => {
  ...
  // ── 3. S2: 场景判断器 ──
  const SCENARIOS = [
    { scene:'为论文 Figure 1 绘制散点图（数据精确）', answer:'no',
  ...
  const quizGrid = document.getElementById('p01-quiz-grid');
  if (quizGrid) {
    SCENARIOS.forEach((sc, idx) => {
      const card = document.createElement('div');
      card.className = 'p01-quiz-card';
      card.innerHTML = `<p class="p01-quiz-scene">${sc.scene}</p>
        <div class="p01-quiz-btns">
          ${['yes','no','assist'].map(k => `<button class="p01-quiz-btn" data-k="${k}" data-correct="${sc.answer}">${LABELS[k]}</button>`).join('')}
        </div>
        <div class="p01-quiz-result" id="p01-qr-${idx}"></div>`;
      quizGrid.appendChild(card);
    });
    quizGrid.addEventListener('click', e => {
      const btn = e.target.closest('.p01-quiz-btn');
      if (!btn || btn.disabled) return;
      const card = btn.closest('.p01-quiz-card');
      const correct = btn.dataset.correct;
      const chosen  = btn.dataset.k;
      const idx     = [...quizGrid.children].indexOf(card);
      const resultEl = document.getElementById(`p01-qr-${idx}`);
      card.querySelectorAll('.p01-quiz-btn').forEach(b => {
        b.disabled = true;
        if (b.dataset.k === correct && b !== btn) b.classList.add('reveal-correct');
      });
      btn.classList.add(chosen === correct ? 'selected-correct' : 'selected-wrong');
      const ok = chosen === correct;
      resultEl.textContent = (ok ? '✓ 正确！' : '✗ 错误。') + ' ' + SCENARIOS[idx].reason;
      resultEl.className = `p01-quiz-result show ${ok ? 'ok' : 'bad'}`;
    });
  }
  fadeIn(document.querySelectorAll('#p01-s2 .reading-wrapper, #p01-s2 .p01-quiz-card'), { stagger:0.07, y:30 });

  // ── 4. S3: 能力矩阵 ──
  const TOOLS = ['GPT Image 1.5','Midjourney v7','FLUX 1.1 Pro','Recraft V3 SVG','StarVector','Mermaid AI'];
  const TASKS = ['概念示意图','流程图','数据图辅助','图标/素材'];
  const SCORES = [[2,1,0,2],[2,0,0,2],[2,0,0,2],[2,1,1,2],[1,2,0,1],[0,2,0,0]];
  const DESCS = [
    ['优秀的概念图生成，风格多样','能生成流程图但精确度有限','不建议生成精确数据图','可生成精美图标'],
    ['艺术风格强，适合综述封面','不擅长结构化流程图','不适合','高质量图标素材'],
    ['开源强模型，科学图示质量优秀','不擅长','不适合','高质量图标素材'],
  ...
  const tooltip  = document.getElementById('p01-tooltip');
  if (matrixEl) {
    const table = document.createElement('table');
    table.className = 'p01-matrix-table';
    table.innerHTML = `<thead><tr><th style="text-align:left;min-width:140px;">工具</th>${TASKS.map(t => `<th>${t}</th>`).join('')}</tr></thead>`;
    const tbody = document.createElement('tbody');
    TOOLS.forEach((tool, ti) => {
      const tr = document.createElement('tr');
      tr.innerHTML = `<td class="p01-tool-label">${tool}</td>` +
        TASKS.map((task, ki) => {
          const s = SCORES[ti][ki];
          const desc = DESCS[ti]?.[ki] || '';
          return `<td><div class="p01-matrix-cell s${s}" data-info="${tool} × ${task}：${desc}">${EMOJI[s]}</div></td>`;
        }).join('');
    ...
    if (tooltip) {
      table.addEventListener('mouseover', e => {
        const cell = e.target.closest('.p01-matrix-cell');
        if (!cell) return;
  ...
  fadeIn(document.querySelectorAll('#p01-s3 .reading-wrapper, #p01-matrix'), { stagger:0.15, y:30 });
...
要求：同心圆或四象限布局，各类型有代表性图标`,
  ...
  if (tabsEl && panelsEl) {
    SHOWCASE.forEach((sc, i) => {
      ...
      tab.className = `p01-sc-tab${i === 0 ? ' active' : ''}`;
    ...
    tabsEl.addEventListener('click', e => {
      const tab = e.target.closest('.p01-sc-tab');
      if (!tab) return;
      tabsEl.querySelectorAll('.p01-sc-tab').forEach(t => t.classList.remove('active'));
      panelsEl.querySelectorAll('.p01-sc-panel').forEach(p => p.classList.remove('active'));
      tab.classList.add('active');
      document.getElementById(`p01-panel-${tab.dataset.idx}`)?.classList.add('active');
    });
  }
  fadeIn(document.querySelectorAll('#p01-s4 .reading-wrapper, #p01-sc-tabs'), { stagger:0.1, y:30 });
