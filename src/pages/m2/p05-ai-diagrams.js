...
const POLICY_DETAILS = {
  'Nature-数据图（Figure）': 'Nature明确禁止使用AI生成任何研究数据图，违反将被视为学术不端（2024年修订政策）。',
  'Nature-示意图/概念图': '允许AI辅助生成概念示意图，但必须在图注中注明"该图由[AI工具名]辅助生成"。',
  'Nature-封面图': '封面图使用AI须提前获得编辑书面批准，审批周期通常3-5个工作日。',
  'Nature-AI文字辅助': '允许使用AI辅助语言润色，但须在Author Contributions部分声明具体用途。',
  'Science-数据图（Figure）': 'Science政策与Nature一致，严格禁止AI生成数据图表，包括修改原始数据的AI增强。',
  'Science-示意图/概念图': '概念图允许，要求图注末尾注明"Figure created with [tool] assistance"。',
  'Science-封面图': '需联系Science编辑委员会，通过专项审核流程，通常需要说明AI工具的具体用途。',
  'Science-AI文字辅助': '须在致谢或方法中说明AI辅助写作工具及使用范围。',
  'Cell/Elsevier-数据图（Figure）': 'Elsevier系列期刊（含Cell）明确禁止AI生成数据图，适用于旗下所有2000+期刊。',
  'Cell/Elsevier-示意图/概念图': '概念图允许并须声明，Elsevier提供标准声明模板供作者使用。',
  'Cell/Elsevier-封面图': 'Cell明确允许AI辅助封面图，但须在封面申请中注明，且创意方向须由作者主导。',
  'Cell/Elsevier-AI文字辅助': 'Elsevier要求在方法或致谢中详细说明使用的具体AI工具和用途。',
  'PNAS-数据图（Figure）': 'PNAS对数据图持严格禁止立场，并特别指出禁止用AI"修复"或"增强"真实数据图像。',
  'PNAS-示意图/概念图': '示意图须声明，PNAS建议在图注中注明生成工具和提示词要点。',
  'PNAS-封面图': 'PNAS不定期征集封面图，AI辅助封面须提前与编辑沟通，个案处理。',
  'PNAS-AI文字辅助': 'PNAS在2024年更新政策，明确允许AI文字辅助，无需额外说明即视为合规。',
  'Lancet-数据图（Figure）': 'Lancet系列期刊对AI持最严格立场，任何涉及临床数据的AI生成内容均被禁止。',
  'Lancet-示意图/概念图': 'Lancet明确禁止AI生成示意图，理由是医学期刊对准确性要求高于其他领域。',
  'Lancet-封面图': 'Lancet封面图须为真实患者/医疗场景摄影，不接受AI生成，包括艺术化处理。',
  'Lancet-AI文字辅助': '允许语言润色，但须在方法中声明，且AI生成文字不得超过正文的10%。',
};

const SPECTRUM_LAYERS = [
  { level: 1, color: '#22c55e', label: '完全安全', example: 'AI辅助配色、排版建议、语法检查', desc: '使用AI工具协助配色决策、排版建议或语法检查，无需额外声明。这类辅助与使用Spell Check本质相同。' },
  { level: 2, color: '#84cc16', label: '需声明使用', example: 'AI生成概念示意图用于正文', desc: '在图注中注明"图由[工具名]辅助生成"，并确保科学准确性由作者保证。大多数期刊接受，需查阅具体投稿指南。' },
  { level: 3, color: '#eab308', label: '需编辑批准', example: 'AI生成期刊封面图', desc: '提交前需联系期刊编辑获得书面批准，不同期刊政策存在显著差异。建议提前2-4周与编辑沟通。' },
  { level: 4, color: '#ef4444', label: '严格禁止', example: 'AI生成或修改实验数据图', desc: '将AI生成内容用于展示数据、实验结果或统计分析，属于学术数据造假，可能导致撤稿和学术处分。' },
  { level: 5, color: '#6b7280', label: '灰色地带', example: 'AI增强真实显微照片', desc: '对真实照片进行AI增强（去噪、分辨率提升）存在争议，需在方法部分详细说明处理过程和软件版本。' },
];

const ETHICS_SCENARIOS = [
  { q: '将遥感卫星图用AI"去云"处理后作为Figure数据展示', answer: 'forbid',
    feedback: '这是对真实数据的AI修改，PNAS等期刊明确禁止，必须在方法中详细说明处理过程，且需经过peer review评估。' },
  { q: '用Midjourney生成论文TOC图形摘要（Graphical Abstract）', answer: 'declare',
    feedback: 'TOC图属于示意图，大多数期刊允许但须声明。格式：图注末尾加 "This figure was generated with AI assistance (Midjourney, 2025)"。' },
  { q: '用ChatGPT润色图表说明（Figure Caption）文字', answer: 'ok',
    feedback: '语言辅助属于允许范围，但Elsevier系列期刊要求在Author Contributions中说明AI工具的具体使用范围。' },
  { q: '用Stable Diffusion生成"理想化"实验装置示意图用于Methods', answer: 'declare',
    feedback: '实验装置示意图允许（如科学准确），须声明AI辅助，并确保所有技术细节经人工核实。注意：不得添加实际实验中不存在的装置组件。' },
  { q: 'AI直接生成扫描电镜（SEM）风格图像替代真实显微照片', answer: 'forbid',
    feedback: '严格禁止。AI生成的显微图像假冒真实数据，属于严重学术不端，可能导致撤稿、调查和处分。' },
  { q: '将ChatGPT生成文字作为论文正文发表而不声明', answer: 'forbid',
    feedback: '几乎所有顶刊要求声明任何AI辅助写作。不声明被认定为学术不端，可能导致拒稿或追溯撤稿。' },
  { q: '用Mermaid AI生成实验流程图在Methods部分使用', answer: 'declare',
    feedback: '流程图用于方法说明合理，须声明"流程图使用Mermaid AI辅助绘制，内容经作者核实"。Mermaid生成的是可编辑结构图，透明度更高。' },
  { q: '申请期刊封面时使用AI生成的河流生态系统渲染图', answer: 'declare',
    feedback: '封面图使用AI须提前联系编辑获批（Nature/Science）或在申请时注明（Cell），不得直接提交而不声明。' },
];

const JOURNALS_LIST = ['Nature', 'Science', 'Cell', 'Elsevier系列', 'PNAS', 'Lancet', 'ACS期刊', 'Wiley期刊', '其他期刊'];
const USAGE_TYPES = ['概念示意图/流程图', 'Graphical Abstract（图形摘要）', '封面图', '数据可视化辅助', '语言润色', '多种用途'];
const AI_TOOLS = ['GPT Image / DALL-E', 'Midjourney', 'Stable Diffusion / FLUX', 'Recraft V3', 'Mermaid AI', 'ChatGPT（文字辅助）', '多种AI工具'];

// ── render ────────────────────────────────────────────────────────────────────

export function render() {
  // 构建期刊政策表格 HTML
  const tableHeaderCells = DIMENSIONS.map(d => `<th>${d}</th>`).join('');
  const tableRows = JOURNALS.map((journal, ji) => {
    const cells = DIMENSIONS.map((dim, di) => {
      const policy = POLICIES[ji][di];
      let cls = '';
      if (policy.includes('❌')) cls = 'p05-cell-no';
      else if (policy.includes('⚠️')) cls = 'p05-cell-warn';
      else if (policy.includes('✅')) cls = 'p05-cell-ok';
      return `<td class="${cls}" data-journal="${journal}" data-dim="${dim}">${policy}</td>`;
    }).join('');
    return `<tr><td>${journal}</td>${cells}</tr>`;
  }).join('');

  // 构建情景练习卡片
  const quizCards = ETHICS_SCENARIOS.map((sc, i) => `
    <div class="p05-quiz-card" data-idx="${i}">
      <p class="p05-quiz-q">${sc.q}</p>
  ...
  // 构建声明生成器下拉选项
  const journalOptions = JOURNALS_LIST.map(j => `<option value="${j}">${j}</option>`).join('');
  const usageOptions = USAGE_TYPES.map(u => `<option value="${u}">${u}</option>`).join('');
  const toolOptions = AI_TOOLS.map(t => `<option value="${t}">${t}</option>`).join('');
...
function generateDeclaration(journal, usage, tool) {
  ...
  const EN = `The ${usage} in this manuscript was created with the assistance of ${tool} (${year}). All content has been reviewed and verified by the authors for scientific accuracy. This use of AI-generated imagery complies with the editorial policies of ${journal}.`;
  const CN = `本文中的${usage}在${tool}辅助下创作（${year}年）。所有内容已由作者审核并核实其科学准确性，符合${journal}的编辑政策要求。`;
  return { EN, CN };
}

function drawSpectrum() {
  const container = document.getElementById('p05-spectrum-wrap');
  if (!container) return;

  SPECTRUM_LAYERS.forEach((layer) => {
    const row = document.createElement('div');
    row.className = 'p05-spectrum-row';
    row.style.cssText = `border-left:4px solid ${layer.color};background:${layer.color}15;`;
    row.innerHTML = `
    ...
    const detailEl = row.querySelector('.p05-spectrum-detail');
    ...
    _scrollHandlers.push({ el: mainEl, type: 'click', fn: handler });
  });
}
...
export function init() {
  ...
  heroTl.fromTo('.p05-hero .p05-hero-tagline', { y: 20, opacity: 0 }, { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out' }, 0.45);
  ...
  // Hero 快捷导航滚动
  document.querySelectorAll('#p05-quicknav .hero-quicknav__item').forEach(btn => {
    const handler = () => {
  ...
  // ── S1 期刊政策表格交互 ──────────────────────────────────────────────────
  const popup = document.getElementById('p05-policy-popup');
  const popupTitle = document.getElementById('p05-popup-title');
  const popupBody = document.getElementById('p05-popup-body');
  const popupClose = document.getElementById('p05-popup-close');

  // 关闭弹窗
  const closePopup = () => { if (popup) popup.style.display = 'none'; };
  if (popupClose) {
    popupClose.addEventListener('click', closePopup);
    _scrollHandlers.push({ el: popupClose, type: 'click', fn: closePopup });
  }

  // 点击外部关闭弹窗
  const docClickClose = (e) => {
    if (popup && popup.style.display !== 'none' && !popup.contains(e.target)) {
      const cell = e.target.closest('td[data-journal]');
      if (!cell) closePopup();
    }
  };
  document.addEventListener('click', docClickClose);
  _scrollHandlers.push({ el: document, type: 'click', fn: docClickClose });

  // 点击表格单元格
  document.querySelectorAll('.p05-policy-table td[data-journal]').forEach(td => {
    const handler = (e) => {
      const journal = td.dataset.journal;
      const dim = td.dataset.dim;
      const key = `${journal}-${dim}`;
      const detail = POLICY_DETAILS[key] || '暂无详细说明。';

      popupTitle.textContent = `${journal} — ${dim}`;
      popupBody.textContent = detail;
    ...
    _scrollHandlers.push({ el: td, type: 'click', fn: handler });
  });
  ...
  // ── S3 情景练习 ─────────────────────────────────────────────────────────
  const answerMap = { ok: '可以用', declare: '需要声明', forbid: '禁止使用' };

  document.querySelectorAll('.p05-quiz-card').forEach((card) => {
    const idx = parseInt(card.dataset.idx, 10);
    const scenario = ETHICS_SCENARIOS[idx];
    const resultEl = card.querySelector('.p05-quiz-result');
    let answered = false;

    card.querySelectorAll('.p05-quiz-btn').forEach(btn => {
      const handler = () => {
        if (answered) return;
        answered = true;
        ...
        // 标记所有按钮
        card.querySelectorAll('.p05-quiz-btn').forEach(b => {
          if (b.dataset.choice === scenario.answer) {
        ...
        // 显示反馈
        resultEl.textContent = (isCorrect ? '✅ 正确！' : `❌ 正确答案是「${answerMap[scenario.answer]}」。`) + ' ' + scenario.feedback;
        resultEl.className = `p05-quiz-result show ${isCorrect ? 'correct-msg' : 'wrong-msg'}`;
      };
      btn.addEventListener('click', handler);
      _scrollHandlers.push({ el: btn, type: 'click', fn: handler });
    });
  ...
  // ── S4 声明生成器 ──────────────────────────────────────────────────────
  const selJournal = document.getElementById('p05-sel-journal');
  const selUsage   = document.getElementById('p05-sel-usage');
  const selTool    = document.getElementById('p05-sel-tool');
  const declCn     = document.getElementById('p05-decl-cn');
  const declEn     = document.getElementById('p05-decl-en');
  const copyBtn    = document.getElementById('p05-copy-btn');

  function updateDeclaration() {
    if (!selJournal || !selUsage || !selTool || !declCn || !declEn) return;
    const { CN, EN } = generateDeclaration(selJournal.value, selUsage.value, selTool.value);
    declCn.textContent = CN;
    declEn.textContent = EN;
  }

  // 初始化文本
  updateDeclaration();

  [selJournal, selUsage, selTool].forEach(sel => {
    if (!sel) return;
    const handler = updateDeclaration;
    sel.addEventListener('change', handler);
    _scrollHandlers.push({ el: sel, type: 'change', fn: handler });
  });

  // 复制按钮
  if (copyBtn) {
    const handler = () => {
      const cn = declCn ? declCn.textContent : '';
      const en = declEn ? declEn.textContent : '';
      const text = `【中文声明】\n${cn}\n\n【English Declaration】\n${en}`;
      navigator.clipboard.writeText(text).then(() => {
        copyBtn.textContent = '已复制 ✓';
        copyBtn.classList.add('copied');
        setTimeout(() => {
          copyBtn.textContent = '复制全部声明';
    ...
    _scrollHandlers.push({ el: copyBtn, type: 'click', fn: handler });
  }

  // ── 滚动渐入动画 ────────────────────────────────────────────────────────
  fadeIn('#p05-s1 .section-eyebrow, #p05-s1 .section-title, #p05-s1 .section-body', { stagger: 0.12, y: 40 });
  fadeIn('#p05-s1 .p05-table-wrap', { stagger: 0, y: 30 });
  fadeIn('#p05-s2 .section-eyebrow, #p05-s2 .section-title, #p05-s2 .section-body', { stagger: 0.12, y: 40 });
  fadeIn('#p05-s3 .section-eyebrow, #p05-s3 .section-title, #p05-s3 .section-body', { stagger: 0.12, y: 40 });
  fadeIn('.p05-quiz-card', { stagger: 0.08, y: 30 });
  fadeIn('#p05-s4 .section-eyebrow, #p05-s4 .section-title, #p05-s4 .section-body', { stagger: 0.12, y: 40 });
  fadeIn('.p05-gen-selects, .p05-gen-output', { stagger: 0.12, y: 30 });
...
export function destroy() {
  ...
  // 清理光谱容器
  const spectrumWrap = document.getElementById('p05-spectrum-wrap');
  if (spectrumWrap) spectrumWrap.innerHTML = '';
}
